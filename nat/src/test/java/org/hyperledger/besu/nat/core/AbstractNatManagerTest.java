...
public class AbstractNatManagerTest {
  ...
  public void assertThatDoStartIsCalledOnlyOnce()
      throws NatInitializationException {
    final AbstractNatManager natManager =
        Mockito.spy(buildNatManager(NatMethod.UPNP));
    natManager.start();
    natManager.start();
  ...
  public void assertThatManagerReturnValidNatMethod() {
    assertThat(buildNatManager(NatMethod.UPNP).getNatMethod())
  ...
  public void assertThatManagerReturnValidLocalIpAddress()
    ...
    assertThat(buildNatManager(NatMethod.UPNP).queryLocalIPAddress().get())
