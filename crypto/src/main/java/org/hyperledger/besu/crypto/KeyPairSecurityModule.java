...
/**
 * Implementation of SecurityModule wrapping SECP256K1.KeyPair and performing
 * cryptographic operations via SECP256K1 class
 */
public class KeyPairSecurityModule implements SecurityModule {
  ...
  public Signature sign(final Bytes32 dataHash) throws SecurityModuleException {
    try {
      ...
      throw new SecurityModuleException(
          "Unexpected error while signing: " + e.getMessage(), e);
    }
  }
  ...
  public Bytes32 calculateECDHKeyAgreement(final PublicKey partyKey)
    ...
    try {
      ...
      final SECP256K1.PublicKey secp256KPartyKey =
          SECP256K1.PublicKey.create(encodedECPoint);
      return SECP256K1.calculateECDHKeyAgreement(keyPair.getPrivateKey(),
                                                 secp256KPartyKey);
    } catch (final Exception e) {
      throw new SecurityModuleException(
          "Unexpected error while calculating ECDH Key Agreement: " +
              e.getMessage(),
          e);
    }
  }
  ...
  private static class PublicKeyImpl implements PublicKey {
    ...
    PublicKeyImpl(final ECPoint w) { this.w = w; }
