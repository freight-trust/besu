...
public class SECP256K1 {
  ...
  public static void enableNative() {
    ...
    LOG.info(useNative ? "Using native secp256k1"
                       : "Native secp256k1 requested but not available");
  }
   ...
   */
  public static boolean verify(final Bytes data, final Signature signature,
                               final PublicKey pub) {
    if (useNative) {
  ...
  private static ECPoint decompressKey(final BigInteger xBN,
                                       final boolean yBit) {
   ...
   *     wasn't possible.
   */
  private static BigInteger recoverFromSignature(final int recId,
                                                 final BigInteger r,
                                                 final BigInteger s,
    ...
    // conversion routine specified in Section 2.3.7, where mlen = ⌈(log2 p)/8⌉
    // or mlen = ⌈m/8⌉. 1.3. Convert the octet string (16 set binary digits)||X
    // to an elliptic curve point R using the conversion routine specified in
    ...
    // via iterating recId) 1.6.1. Compute a candidate public key as: Q = mi(r)
    // * (sR - eG)
    //
    // Where mi(x) is the modular multiplicative inverse. We transform this into
    // the following: Q = (mi(r) * s ** R) + (mi(r) * -e ** G) Where -e is the
    // modular additive inverse of e, that is z such that z + e = 0 (mod n). In
    // the above equation ** is point multiplication and + is point addition
    // (the EC group operator).
    //
  ...
  private static Signature signDefault(final Bytes32 dataHash,
    ...
    final ECPrivateKeyParameters privKey = new ECPrivateKeyParameters(
        keyPair.getPrivateKey().getEncodedBytes().toUnsignedBigInteger(),
        CURVE);
    signer.init(true, privKey);

    final BigInteger[] components =
        signer.generateSignature(dataHash.toArrayUnsafe());

    return normaliseSignature(components[0], components[1],
                              keyPair.getPublicKey(), dataHash);
  }

  public static Signature normaliseSignature(final BigInteger nativeR,
                                             final BigInteger nativeS,
                                             final PublicKey publicKey,
                                             final Bytes32 dataHash) {

    BigInteger s = nativeS;
    // Automatically adjust the S component to be less than or equal to half the
    // curve order, if necessary. This is required because for every signature
    ...
    // invariants. Thus in future only one of those forms will be considered
    ...
    if (s.compareTo(HALF_CURVE_ORDER) > 0) {
      ...
      // bring it back to the lower half. Otherwise, imagine that N = 10 s = 8,
      // so (-8 % 10 == 2) thus both (r, 8) and (r, 2) are valid solutions. 10 -
      // 8 == 2, giving us always the latter solution, which is canonical.
    ...
    final BigInteger publicKeyBI =
        publicKey.getEncodedBytes().toUnsignedBigInteger();
    for (int i = 0; i < 4; i++) {
  ...
  private static boolean verifyDefault(final Bytes data,
                                       final Signature signature,
                                       final PublicKey pub) {
    final ECDSASigner signer = new ECDSASigner();
    final Bytes toDecode = Bytes.wrap(Bytes.of((byte)4), pub.getEncodedBytes());
    final ECPublicKeyParameters params = new ECPublicKeyParameters(
        CURVE.getCurve().decodePoint(toDecode.toArrayUnsafe()), CURVE);
    signer.init(false, params);
    try {
      return signer.verifySignature(data.toArrayUnsafe(), signature.r,
                                    signature.s);
    } catch (final NullPointerException e) {
      // Bouncy Castle contains a bug that can cause NPEs given specially
      // crafted signatures. Those signatures are inherently invalid/attack sigs
      // so we just fail them here rather than crash the thread.
      return false;
    }
  }

  /**
   * Verifies the given ECDSA signature using the public key bytes against the
   * message bytes, previously passed through a preprocessor function, which is
   * normally a hashing function.
   *
   * @param data The data to verify.
   * @param signature ASN.1 encoded signature.
   * @param pub The public key bytes to use.
   * @param preprocessor The function to apply to the data before verifying the
   *     signature, normally a hashing function.
   * @return True if the verification is successful.
   */
  public static boolean verify(final Bytes data, final Signature signature,
                               final PublicKey pub,
  ...
  public static Bytes32 calculateECDHKeyAgreement(final PrivateKey privKey,
    ...
    final ECPrivateKeyParameters privKeyP =
        new ECPrivateKeyParameters(privKey.getD(), CURVE);
    final ECPublicKeyParameters pubKeyP =
        new ECPublicKeyParameters(theirPubKey.asEcPoint(), CURVE);
  ...
  private static Signature signNative(final Bytes32 dataHash,
                                      final KeyPair keyPair) {
    ...
    if (LibSecp256k1.secp256k1_ecdsa_sign_recoverable(
            LibSecp256k1.CONTEXT, signature, dataHash.toArrayUnsafe(),
            keyPair.privateKey.getEncoded(), null, null) == 0) {
      throw new RuntimeException(
    ...
    return Signature.create(r.toUnsignedBigInteger(), s.toUnsignedBigInteger(),
                            (byte)recId.getValue());
  }

  private static boolean verifyNative(final Bytes data,
                                      final Signature signature,
                                      final PublicKey pub) {

    // translate signature
    final LibSecp256k1.secp256k1_ecdsa_signature _signature =
        new secp256k1_ecdsa_signature();
    if (LibSecp256k1.secp256k1_ecdsa_signature_parse_compact(
            LibSecp256k1.CONTEXT, _signature,
            signature.encodedBytes().toArrayUnsafe()) == 0) {
    ...
    final Bytes encodedPubKey =
        Bytes.concatenate(Bytes.of(0x04), pub.getEncodedBytes());
    if (LibSecp256k1.secp256k1_ec_pubkey_parse(LibSecp256k1.CONTEXT, _pub,
                                               encodedPubKey.toArrayUnsafe(),
                                               encodedPubKey.size()) == 0) {
      throw new IllegalArgumentException("Could not parse public key");
    }

    return LibSecp256k1.secp256k1_ecdsa_verify(LibSecp256k1.CONTEXT, _signature,
                                               data.toArrayUnsafe(), _pub) != 0;
  }

  private static Optional<PublicKey>
  recoverFromSignatureNative(final Bytes32 dataHash,
                             final Signature signature) {
    ...
    if (LibSecp256k1.secp256k1_ecdsa_recoverable_signature_parse_compact(
            LibSecp256k1.CONTEXT, parsedSignature,
            encodedSig.slice(0, 64).toArrayUnsafe(), encodedSig.get(64)) == 0) {
      throw new IllegalArgumentException("Could not parse signature");
    }

    // recover the key
    final LibSecp256k1.secp256k1_pubkey newPubKey =
        new LibSecp256k1.secp256k1_pubkey();
    if (LibSecp256k1.secp256k1_ecdsa_recover(LibSecp256k1.CONTEXT, newPubKey,
                                             parsedSignature,
                                             dataHash.toArrayUnsafe()) == 0) {
    ...
    LibSecp256k1.secp256k1_ec_pubkey_serialize(LibSecp256k1.CONTEXT,
                                               recoveredKey, keySize, newPubKey,
                                               SECP256K1_EC_UNCOMPRESSED);

    return Optional.of(
        PublicKey.create(Bytes.wrapByteBuffer(recoveredKey).slice(1)));
  }
  ...
  public static class PrivateKey implements java.security.PrivateKey {
    ...
    public Bytes32 getEncodedBytes() { return encoded; }

    public BigInteger getD() { return encoded.toUnsignedBigInteger(); }
  ...
  public static class PublicKey implements java.security.PublicKey {
    ...
    public static PublicKey create(final PrivateKey privateKey) {
      ...
      return PublicKey.create(
          Bytes.wrap(Arrays.copyOfRange(point.getEncoded(false), 1, 65)));
    }
    ...
    public static Optional<PublicKey>
    recoverFromSignature(final Bytes32 dataHash, final Signature signature) {
      if (useNative) {
        return recoverFromSignatureNative(dataHash, signature);
      } else {
        final BigInteger publicKeyBI = SECP256K1.recoverFromSignature(
    ...
    private PublicKey(final Bytes encoded) {
      checkNotNull(encoded);
      checkArgument(encoded.size() == BYTE_LENGTH,
                    "Encoding must be %s bytes long, got %s", BYTE_LENGTH,
                    encoded.size());
      this.encoded = encoded;
    }

    /**
     * Returns this public key as an {@link ECPoint} of Bouncy Castle, to
     * facilitate cryptographic operations.
     *
    ...
    public Bytes getEncodedBytes() { return encoded; }
  ...
  public static class KeyPair {
    ...
    public static KeyPair generate() {
      final java.security.KeyPair rawKeyPair =
      ...
      // for details Additionally, as the first bit is a constant prefix (0x04)
      ...
      return new KeyPair(PrivateKey.create(privateKeyValue),
                         PublicKey.create(publicKeyValue));
    }
  ...
  public static class Signature {
    ...
    /**
     * The recovery id to reconstruct the public key used to create the
     * signature.
     *
    ...
    private final Supplier<Bytes> encoded =
        Suppliers.memoize(this::_encodedBytes);
     ...
     * @throws IllegalArgumentException if any argument is invalid (for
     *     instance, {@code v} is neither 27 or 28).
     */
    ...
    private static void checkInBounds(final String name, final BigInteger i) {
      if (i.compareTo(BigInteger.ONE) < 0) {
        throw new IllegalArgumentException(String.format(
            "Invalid '%s' value, should be >= 1 but got %s", name, i));
      }

      if (i.compareTo(CURVE_ORDER) >= 0) {
        throw new IllegalArgumentException(
            String.format("Invalid '%s' value, should be < %s but got %s",
                          CURVE_ORDER, name, i));
      }
    }

    public static Signature decode(final Bytes bytes) {
      checkArgument(bytes.size() == BYTES_REQUIRED,
    ...
    public Bytes encodedBytes() { return encoded.get(); }
    ...
    public boolean equals(final Object other) {
      ...
      return this.r.equals(that.r) && this.s.equals(that.s) &&
    ...
    public byte getRecId() { return recId; }
