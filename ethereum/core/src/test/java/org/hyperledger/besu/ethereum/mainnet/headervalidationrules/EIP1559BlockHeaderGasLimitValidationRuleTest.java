...
public class EIP1559BlockHeaderGasLimitValidationRuleTest {
    ...
    assertThatThrownBy(
        ()
            -> validationRule.validate(
                blockHeader(FORK_BLOCK - 1, 0, Optional.empty()), null))
  ...
  public void shouldReturnTrueBeforeFork() {
    enableEIP1559();
    assertThat(validationRule.validate(
                   blockHeader(FORK_BLOCK - 1, 0, Optional.empty()), null))
        .isTrue();
  }
  ...
  public void shouldReturnTrueIfMaxGasLimitAfterFinalizedFork() {
    enableEIP1559();
    assertThat(validationRule.validate(blockHeader(finalizedForkBlock + 1, 0,
                                                   Optional.empty(),
                                                   feeMarket.getMaxGas()),
                                       null))
        .isTrue();
  ...
  public void shouldReturnFalseIfInvalidGasLimitAfterFork() {
    enableEIP1559();
    assertThat(
        validationRule.validate(blockHeader(FORK_BLOCK + 1, 0, Optional.empty(),
                                            feeMarket.getMaxGas() - 1),
                                null))
        .isFalse();
  }
