...
public class MarkSweepPrunerTest {
  ...
  public void mark_marksAllExpectedNodes() {
    final MarkSweepPruner pruner = new MarkSweepPruner(
        worldStateStorage, blockchain, markStorage, metricsSystem);
    ...
    final BlockHeader markBlock =
        blockchain.getBlockHeader(markBlockNumber).get();
    // Collect the nodes we expect to keep
    final Set<Bytes> expectedNodes =
        collectWorldStateNodes(markBlock.getStateRoot());
    assertThat(hashValueStore.size())
        .isGreaterThan(expectedNodes.size()); // Sanity check

    // Mark and sweep
    pruner.mark(markBlock.getStateRoot());
    pruner.sweepBefore(markBlock.getNumber());

    // Assert that the block we marked is still present and all accounts are
    // accessible
    assertThat(worldStateArchive.get(markBlock.getStateRoot())).isPresent();
    final WorldState markedState =
        worldStateArchive.get(markBlock.getStateRoot()).get();
    ...
    assertThat(hashValueStore.values())
        .containsExactlyInAnyOrderElementsOf(expectedNodes.stream()
  ...
  public void sweepBefore_shouldSweepStateRootFirst() {
    final MarkSweepPruner pruner = new MarkSweepPruner(
        worldStateStorage, blockchain, markStorage, metricsSystem, 1);
