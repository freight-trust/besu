...
public final class ProofOfWorkValidationRule
    implements DetachedBlockHeaderValidationRule {

  private static final Logger LOG = LogManager.getLogger();

  private static final BigInteger ETHASH_TARGET_UPPER_BOUND =
  ...
  public ProofOfWorkValidationRule() { this(false); }
  ...
  public boolean validate(final BlockHeader header, final BlockHeader parent) {
    if (includeBaseFee) {
      if (!ExperimentalEIPs.eip1559Enabled) {
        LOG.warn(
            "Invalid block header: EIP-1559 feature flag must be enabled --Xeip1559-enabled");
        return false;
    ...
    HASHER.hash(hashBuffer, header.getNonce(), header.getNumber(),
                headerHash.toArray());
    ...
    final BigInteger difficulty =
        header.getDifficulty().toBytes().toUnsignedBigInteger();
    ...
    final Hash mixedHash = Hash.wrap(
    ...
    if (!header.getMixHash().equals(mixedHash)) {
      LOG.warn(
          "Invalid block header: header mixed hash {} does not equal calculated mixed hash {}.\n"
              + "Failing header:\n{}",
          header.getMixHash(), mixedHash, header);
      return false;
    }
