...
public class BaseFee {
  ...
  public BaseFee(final FeeMarket feeMarket, final long value) {
    this.value = value;
    this.delta =
        Math.floorDiv(value, feeMarket.getBasefeeMaxChangeDenominator());
  ...
  public static Wei minTransactionPriceInNextBlock(
    ...
    if (baseFee.isPresent()) {
      minBaseFeeInNextBlock = Optional.of(
          new BaseFee(FeeMarket.eip1559(), baseFee.get()).getMinNextValue());
    }
  ...
  public long getMinNextValue() { return minNextValue; }

  public long getMaxNextValue() { return maxNextValue; }
}
