...
/**
 * A transaction receipt for a private transaction, containing information
 * pertaining a transaction execution.
 */
public class PrivateTransactionReceipt {

  @SuppressWarnings("unchecked")
  public static final PrivateTransactionReceipt FAILED =
      new PrivateTransactionReceipt(0, Collections.EMPTY_LIST, Bytes.EMPTY,
   ...
   *
   * @param status the state root for the world state after the transaction has
   *     been processed
   * @param logs the total amount of gas consumed in the block after this
   *     transaction
   * @param output output from the transaction
   * @param revertReason the revert reason for a failed transaction (if
   *     applicable)
   */
  public PrivateTransactionReceipt(final int status, final List<Log> logs,
                                   final Bytes output,
  ...
  public PrivateTransactionReceipt(final TransactionProcessor.Result result) {
    this(result.getStatus() ==
             ...
             ? 1
             : 0,
         result.getLogs(), result.getOutput(), result.getRevertReason());
  }
   ...
   */
  public Bytes getOutput() { return output; }
  ...
  public boolean equals(final Object obj) {
    ...
    return logs.equals(other.getLogs()) && status == other.status &&
            output.equals(other.output) && revertReason.isPresent()
