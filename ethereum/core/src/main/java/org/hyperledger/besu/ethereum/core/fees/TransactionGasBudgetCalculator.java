...
public interface TransactionGasBudgetCalculator {
  ...
  boolean hasBudget(final Transaction transaction,
                    final BlockHeader blockHeader, final long gasUsed);

  static TransactionGasBudgetCalculator frontier() {
    return gasBudgetCalculator(
        (blockHeader, ignored) -> blockHeader.getGasLimit());
  }
  ...
  static TransactionGasBudgetCalculator
  gasBudgetCalculator(final BlockGasLimitCalculator blockGasLimitCalculator) {
    return (transaction, blockHeader, gasUsed) -> {
      ...
      return Long.compareUnsigned(transaction.getGasLimit(),
                                  remainingGasBudget) <= 0;
    };
  }
