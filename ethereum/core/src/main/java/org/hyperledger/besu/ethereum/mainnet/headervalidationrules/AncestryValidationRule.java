...
/**
 * Ensures the hash of the parent block matches that specified in the parent
 * hash of the proposed header.
 */
public class AncestryValidationRule
  ...
  public boolean validate(final BlockHeader header, final BlockHeader parent) {
    if (!header.getParentHash().equals(parent.getHash())) {
      LOG.trace("Invalid parent block header.  Parent hash {} does not match "
                    + "supplied parent header {}.",
                header.getParentHash(), parent.getHash());
      return false;
    }
