...
public class AltBN128MulPrecompiledContract
  ...
  private static final BigInteger MAX_N = new BigInteger(
      "115792089237316195423570985008687907853269984665640564039457584007913129639935");

  private final Gas gasCost;

  private AltBN128MulPrecompiledContract(final GasCalculator gasCalculator,
                                         final Gas gasCost) {
    super("AltBN128Mul", gasCalculator);
    this.gasCost = gasCost;
  }

  public static AltBN128MulPrecompiledContract
  ...
  private static Bytes computeNative(final Bytes input) {
    ...
    if (altbn128_mul_precompiled(input.toArrayUnsafe(), input.size(), output,
                                 outputSize) == 0) {
