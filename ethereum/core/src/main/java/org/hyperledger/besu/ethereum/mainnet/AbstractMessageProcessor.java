 ...
 *
 * <p>The following methods have been created to be invoked when the message
 * state changes via the
 * {@link MessageFrame.State}. Note that some of these methods are abstract
 * while others have default behaviors. There is currently no method for
 ...
 * <td>{@link AbstractMessageProcessor#start(MessageFrame,
 * OperationTracer)}</td>
 * </tr>
 * <tr>
 * <td>{@link MessageFrame.State#CODE_EXECUTING}</td>
 * <td>{@link AbstractMessageProcessor#codeExecute(MessageFrame,
 * OperationTracer)}</td>
 * </tr>
 * <tr>
 * <td>{@link MessageFrame.State#CODE_SUCCESS}</td>
 * <td>{@link AbstractMessageProcessor#codeSuccess(MessageFrame,
 * OperationTracer)}</td>
 * </tr>
 * <tr>
...
public abstract class AbstractMessageProcessor {
  ...
  private void
  clearAccumulatedStateBesidesGasAndOutput(final MessageFrame frame) {
    final Collection<Address> addressesToForceCommit =
        frame.getWorldState()
            .getTouchedAccounts()
            .stream()
            .filter(a
                    -> forceDeleteAccountsWhenEmpty.contains(a.getAddress()) &&
                           a.isEmpty())
   ...
   * @param operationTracer The tracer recording execution
   */
  private void codeExecute(final MessageFrame frame,
                           final OperationTracer operationTracer) {
    try {
  ...
  public void process(final MessageFrame frame,
                      final OperationTracer operationTracer) {
