...
public class MainnetMessageCallProcessor extends AbstractMessageProcessor {
  ...
  public MainnetMessageCallProcessor(
      final EVM evm, final PrecompileContractRegistry precompiles,
  ...
  public void start(final MessageFrame frame,
    ...
    try {
      ...
      final PrecompiledContract precompile = precompiles.get(
          frame.getContractAddress(), frame.getContractAccountVersion());
      if (precompile != null) {
      ...
      LOG.trace(
          "Message call error: illegal modification not allowed from private state");
...
>>>>>>> master
   ...
   *
   * <p>Assumes that the transaction has been validated so that the sender has
   * the required fund as of the world state of this executor.
   */
  private void transferValue(final MessageFrame frame) {
    ...
    // The yellow paper explicitly states that if the recipient account doesn't
    // exist at this point, it is created.
    final MutableAccount recipientAccount =
        frame.getWorldState()
            .getOrCreate(frame.getRecipientAddress())
            .getMutable();

    if (frame.getRecipientAddress().equals(frame.getSenderAddress())) {
      LOG.trace("Message call of {} to itself: no fund transferred",
                frame.getSenderAddress());
    } else {
      final Wei prevSenderBalance =
          senderAccount.decrementBalance(frame.getValue());
      final Wei prevRecipientBalance =
          recipientAccount.incrementBalance(frame.getValue());

      LOG.trace(
          "Transferred value {} for message call from {} ({} -> {}) to {} ({} -> {})",
          frame.getValue(), frame.getSenderAddress(), prevSenderBalance,
          senderAccount.getBalance(), frame.getRecipientAddress(),
          prevRecipientBalance, recipientAccount.getBalance());
    }
  }

  /**
   * Executes this message call knowing that it is a call to the provide
   * pre-compiled contract.
   *
   * @param contract The contract this is a message call to.
   */
  private void executePrecompile(final PrecompiledContract contract,
    ...
    if (frame.getRemainingGas().compareTo(gasRequirement) < 0) {
      LOG.trace(
          ...
          contract, gasRequirement, frame.getRemainingGas());
      frame.setState(MessageFrame.State.EXCEPTIONAL_HALT);
    } else {
