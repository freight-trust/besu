...
class PendingTransactionsMessageSender {
  ...
  public void sendTransactionsToPeers() {
    StreamSupport
        .stream(transactionTracker.getEthPeersWithUnsentTransactions()
                    .spliterator(),
                true)
        .parallel()
  ...
  private void sendTransactionsToPeer(final EthPeer peer) {
    final Set<Hash> allTxToSend =
        transactionTracker.claimTransactionsToSendToPeer(peer);
    while (!allTxToSend.isEmpty()) {
      final LimitedNewPooledTransactionHashesMessages
          limitedTransactionsMessages =
              LimitedNewPooledTransactionHashesMessages.createLimited(
                  allTxToSend);
      allTxToSend.removeAll(
