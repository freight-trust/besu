...
public class CompleteTaskStep {
  ...
  public CompleteTaskStep(final WorldStateStorage worldStateStorage,
                          final MetricsSystem metricsSystem,
                          final LongSupplier
                              worldStatePendingRequestsCurrentSupplier) {
    this.worldStateStorage = worldStateStorage;
    this.worldStatePendingRequestsCurrentSupplier =
        worldStatePendingRequestsCurrentSupplier;
    completedRequestsCounter = new RunnableCounter(
        ...
        this::displayWorldStateSyncProgress, DISPLAY_PROGRESS_STEP);
    retriedRequestsCounter = metricsSystem.createCounter(
        BesuMetricCategory.SYNCHRONIZER, "world_state_retried_requests_total",
        "Total number of node data requests repeated as part of fast sync world state download");
  }

  public void markAsCompleteOrFailed(final BlockHeader header,
    ...
    if (task.getData().getData() != null) {
      ...
      task.markFailed();
      // Marking the task as failed will add it back to the queue so make sure
      // any threads waiting to read from the queue are notified.
  ...
  long getCompletedRequests() { return completedRequestsCounter.get(); }

  long getPendingRequests() {
    return worldStatePendingRequestsCurrentSupplier.getAsLong();
  }

  private void enqueueChildren(final Task<NodeDataRequest> task,
    ...
    if (!downloadState.downloadWasResumed() ||
        !isRootState(blockHeader, request)) {
      downloadState.enqueueRequests(request.getChildRequests());
    }
  }

  private boolean isRootState(final BlockHeader blockHeader,
