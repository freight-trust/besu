...
public class PeerPendingTransactionTracker
    implements EthPeer.DisconnectCallback {
  private static final int MAX_TRACKED_SEEN_TRANSACTIONS = 10_000;
  private final Map<EthPeer, Set<Hash>> seenTransactions =
      new ConcurrentHashMap<>();
  private final Map<EthPeer, Set<Hash>> transactionsToSend =
      new ConcurrentHashMap<>();
  ...
  markTransactionsHashesAsSeen(final EthPeer peer,
                               final Collection<Hash> transactions) {
    final Set<Hash> seenTransactionsForPeer =
        getOrCreateSeenTransactionsForPeer(peer);
    transactions.stream().forEach(seenTransactionsForPeer::add);
  }

  public synchronized void addToPeerSendQueue(final EthPeer peer,
                                              final Hash hash) {
    if (!hasPeerSeenTransaction(peer, hash)) {
      transactionsToSend.computeIfAbsent(peer, key -> createTransactionsSet())
          .add(hash);
    }
  }
  ...
  public synchronized Set<Hash>
  claimTransactionsToSendToPeer(final EthPeer peer) {
    ...
    if (transactionsToSend != null) {
      markTransactionsHashesAsSeen(
          ...
          transactionsToSend.stream()
              .filter(
                  h -> pendingTransactions.getTransactionByHash(h).isPresent())
  ...
  private Set<Hash> getOrCreateSeenTransactionsForPeer(final EthPeer peer) {
    return seenTransactions.computeIfAbsent(peer,
                                            key -> createTransactionsSet());
  }
  ...
  private boolean hasPeerSeenTransaction(final EthPeer peer, final Hash hash) {
    ...
    return seenTransactionsForPeer != null &&
        seenTransactionsForPeer.contains(hash);
  }
