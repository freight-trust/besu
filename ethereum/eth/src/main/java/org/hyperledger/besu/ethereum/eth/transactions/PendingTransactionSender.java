...
class PendingTransactionSender implements TransactionBatchAddedListener {
  ...
  public void onTransactionsAdded(final Iterable<Transaction> transactions) {
    ethContext.getEthPeers()
        ...
        .filter(
            peer -> transactionTracker.isPeerSupported(peer, EthProtocol.ETH65))
        .forEach(
            peer
            -> transactions.forEach(transaction
                                    -> transactionTracker.addToPeerSendQueue(
                                        peer, transaction.getHash())));
    ethContext.getScheduler().scheduleSyncWorkerTask(
        transactionsMessageSender::sendTransactionsToPeers);
  }
}
