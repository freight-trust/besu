...
public class PendingTransactions {
  ...
  public PendingTransactions(final int maxTransactionRetentionHours,
  ...
  /*
   * The BlockTransaction selection process (part of block mining) requires
   * synchronised access to all pendingTransactions - this allows it to iterate
   * over the available transactions without releasing the lock in between
   * items.
   *
   */
  ...
  private boolean shouldReplace(final TransactionInfo existingTransaction,
                                final TransactionInfo newTransaction) {
    return newTransaction.getTransaction().getGasPrice().compareTo(
               existingTransaction.getTransaction().getGasPrice()) > 0;
  }
    ...
    synchronized (pendingTransactions) {
      ...
      if (transactionsForSenderInfo == null ||
        ...
        return OptionalLong.of(
            Objects.requireNonNull(transactionsForSenderInfo.getGaps().poll()));
      } else {
        return OptionalLong.of(
  ...
  public Collection<Hash> getNewPooledHashes() { return newPooledHashes; }
