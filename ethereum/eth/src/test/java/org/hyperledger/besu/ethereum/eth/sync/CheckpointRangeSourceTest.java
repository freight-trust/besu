...
public class CheckpointRangeSourceTest {
  ...
  private final CheckpointHeaderFetcher checkpointFetcher =
      mock(CheckpointHeaderFetcher.class);
  ...
  private final CheckpointRangeSource source = new CheckpointRangeSource(
      checkpointFetcher, syncTargetChecker, ethScheduler, peer, commonAncestor,
      CHECKPOINT_TIMEOUTS_PERMITTED, Duration.ofMillis(1));

  @Test
  public void
  shouldHaveNextWhenNoCheckpointsLoadedButSyncTargetCheckerSaysToContinue() {
    when(syncTargetChecker.shouldContinueDownloadingFromSyncTarget(
             peer, commonAncestor))
        .thenReturn(true);
  ...
  @Test
  public void
  shouldHaveNextWhenMoreCheckpointsAreLoadedRegardlessOfSyncTargetChecker() {
  ...
  @Test
  public void
  shouldNotHaveNextWhenNoCheckpointsLoadedAndSyncTargetCheckerReturnsFalse() {
  ...
  @Test
  public void
  shouldNotHaveNextWhenNoMoreCheckpointsAvailableAndRetryLimitReached() {
    when(
        syncTargetChecker.shouldContinueDownloadingFromSyncTarget(any(), any()))
        .thenReturn(true);
    when(checkpointFetcher.getNextCheckpointHeaders(peer, commonAncestor))
    ...
    for (int i = 1; i <= CHECKPOINT_TIMEOUTS_PERMITTED; i++) {
      ...
      verify(checkpointFetcher, times(i))
  ...
  public void
  shouldDelayBeforeRetryingRequestForCheckpointHeadersAfterEmptyResponse() {
    when(
        syncTargetChecker.shouldContinueDownloadingFromSyncTarget(any(), any()))
        .thenReturn(true);
    when(checkpointFetcher.getNextCheckpointHeaders(peer, commonAncestor))
    ...
    verify(checkpointFetcher).getNextCheckpointHeaders(peer, commonAncestor);
    verify(ethScheduler)
        .scheduleFutureTask(any(Supplier.class), eq(RETRY_DELAY_DURATION));
  }

  @Test
  @SuppressWarnings("unchecked")
  public void
  shouldDelayBeforeRetryingRequestForCheckpointHeadersAfterFailure() {
    when(
        syncTargetChecker.shouldContinueDownloadingFromSyncTarget(any(), any()))
        .thenReturn(true);
    when(checkpointFetcher.getNextCheckpointHeaders(peer, commonAncestor))
  ...
  public void shouldResetCheckpointFailureCountWhenMoreCheckpointsReceived() {
    ...
    // Failure count should have been reset by the success so limit hasn't been
    // reached.
    assertThat(source).hasNext();
  }
  ...
  public void shouldRequestMoreHeadersWhenCurrentSetHasRunOut() {
    ...
    assertThat(source.next())
        .isEqualTo(new CheckpointRange(peer, commonAncestor, header(15)));
    verify(checkpointFetcher).getNextCheckpointHeaders(peer, commonAncestor);
    verify(checkpointFetcher)
        .nextCheckpointEndsAtChainHead(peer, commonAncestor);

    assertThat(source.next())
        .isEqualTo(new CheckpointRange(peer, header(15), header(20)));
    verifyNoMoreInteractions(checkpointFetcher);

    assertThat(source.next())
        .isEqualTo(new CheckpointRange(peer, header(20), header(25)));
    ...
    assertThat(source.next())
        .isEqualTo(new CheckpointRange(peer, header(25), header(30)));
    verifyNoMoreInteractions(checkpointFetcher);
  }
  ...
  public void shouldSendNewRequestIfRequestForHeadersFails() {
    when(checkpointFetcher.getNextCheckpointHeaders(peer, commonAncestor))
        .thenReturn(
            CompletableFuture.failedFuture(new NoAvailablePeersException()))
        .thenReturn(completedFuture(asList(header(15), header(20))));
    ...
    assertThat(source.next())
        .isEqualTo(new CheckpointRange(peer, commonAncestor, header(15)));
    verify(checkpointFetcher, times(2))
        .getNextCheckpointHeaders(peer, commonAncestor);
  }

  @Test
  public void
  shouldReturnUnboundedCheckpointRangeWhenNextCheckpointEndsAtChainHead() {
    when(syncTargetChecker.shouldContinueDownloadingFromSyncTarget(
             peer, commonAncestor))
        .thenReturn(true);
    when(checkpointFetcher.nextCheckpointEndsAtChainHead(peer, commonAncestor))
        .thenReturn(true);

    assertThat(source).hasNext();
    assertThat(source.next())
        .isEqualTo(new CheckpointRange(peer, commonAncestor));
