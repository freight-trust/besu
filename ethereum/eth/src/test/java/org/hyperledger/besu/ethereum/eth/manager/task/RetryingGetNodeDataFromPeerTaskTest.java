...
public class RetryingGetNodeDataFromPeerTaskTest
    extends RetryingMessageTaskTest<Map<Hash, Bytes>> {
  ...
  protected Map<Hash, Bytes> generateDataToBeRequested() {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      requestedData.put(Hash.hash(protocolContext.getWorldStateArchive()
  ...
  @Override
  protected EthTask<Map<Hash, Bytes>>
  createTask(final Map<Hash, Bytes> requestedData) {
  ...
  @Override
  @Ignore(
      "Partial responses are enough to complete the request so this test doesn't apply")
  public void
  failsWhenPeerReturnsPartialResultThenStops() {}
