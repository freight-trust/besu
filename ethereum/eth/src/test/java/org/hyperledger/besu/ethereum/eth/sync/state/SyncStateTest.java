...
public class SyncStateTest {
  ...
  private static final long TARGET_CHAIN_HEIGHT =
      OUR_CHAIN_HEAD_NUMBER + TARGET_CHAIN_DELTA;
  ...
  public void isInSync_singlePeerWithWorseChainBetterHeight() {
    updateChainState(otherPeer.getEthPeer(), TARGET_CHAIN_HEIGHT,
                     OUR_CHAIN_DIFFICULTY.subtract(1L));
  ...
  public void isInSync_singlePeerWithWorseChainWorseHeight() {
    updateChainState(otherPeer.getEthPeer(), OUR_CHAIN_HEAD_NUMBER - 1L,
                     OUR_CHAIN_DIFFICULTY.subtract(1L));
    final EthPeer peer = mockWorseChain(otherPeer.getEthPeer());
    doReturn(Optional.of(peer)).when(ethPeers).bestPeerWithHeightEstimate();
  ...
  public void isInSync_inSyncWithTargetOutOfSyncWithBestPeer() {
    ...
    final long heightDifference = 20L;
    updateChainState(otherPeer.getEthPeer(),
  ...
  public void shouldBecomeInSyncWhenOurBlockchainCatchesUp() {
    ...
    // We should register as in-sync with default tolerance, out-of-sync with
    // exact tolerance
    assertThat(syncState.isInSync()).isTrue();
    assertThat(syncState.isInSync(0)).isFalse();
    verify(inSyncListener).onInSyncStatusChange(true);
    ...
    advanceLocalChain(TARGET_CHAIN_HEIGHT -
                      Synchronizer.DEFAULT_IN_SYNC_TOLERANCE + 1);
    // We should register as in-sync with default tolerance, out-of-sync with
    // exact tolerance
    assertThat(syncState.isInSync()).isTrue();
    assertThat(syncState.isInSync(0)).isFalse();
  ...
  public void addInSyncListener_whileOutOfSync() {
    ...
    // Fall out of sync
    updateChainState(syncTargetPeer.getEthPeer(),
                     TARGET_CHAIN_HEIGHT +
                         Synchronizer.DEFAULT_IN_SYNC_TOLERANCE + 1L,
                     TARGET_DIFFICULTY.add(10L));

    final ArgumentCaptor<Boolean> inSyncEventCaptor =
        ArgumentCaptor.forClass(Boolean.class);
    verify(newListener, times(3))
        .onInSyncStatusChange(inSyncEventCaptor.capture());
  ...
  public void removeInSyncListener_addAdditionalListenerBeforeRemoving() {
    ...
    final long subscriberId =
        syncState.subscribeInSync(listenerToRemove, syncTolerance);
  ...
  public void syncStatusListener_receivesEventWhenSyncTargetSet() {
    syncState.setSyncTarget(syncTargetPeer.getEthPeer(),
                            blockchain.getBlockHeader(3L).get());
    ...
    assertThat(syncStatus.get().getCurrentBlock())
        .isEqualTo(OUR_CHAIN_HEAD_NUMBER);
    assertThat(syncStatus.get().getHighestBlock())
        .isEqualTo(TARGET_CHAIN_HEIGHT);
  }
  ...
  public void syncStatusListener_receivesEventWhenSyncTargetCleared() {
    ...
    verify(syncStatusListener, times(2))
        .onSyncStatusChanged(syncStatusCaptor.capture());
    ...
    assertThat(syncingEvent.get().getCurrentBlock())
        .isEqualTo(OUR_CHAIN_HEAD_NUMBER);
    assertThat(syncingEvent.get().getHighestBlock())
        .isEqualTo(TARGET_CHAIN_HEIGHT);
  ...
  private RespondingEthPeer createPeer(final Difficulty totalDifficulty,
                                       final long blockHeight) {
    return EthProtocolManagerTestUtil.createPeer(ethProtocolManager,
                                                 totalDifficulty, blockHeight);
  }
  ...
  private EthPeer mockChainIsBetterThan(final EthPeer peer,
  ...
  private void setupOutOfSyncState() {
    syncState.setSyncTarget(syncTargetPeer.getEthPeer(),
                            blockchain.getGenesisBlock().getHeader());
  ...
  /**
   * Updates the chain state, such that the peer will end up with an estimated
   * height of {@code blockHeight} and an estimated total difficulty of {@code
   * totalDifficulty}
   *
  ...
  private void updateChainState(final EthPeer peer, final long blockHeight,
    ...
    assertThat(peer.chainState().getEstimatedTotalDifficulty())
