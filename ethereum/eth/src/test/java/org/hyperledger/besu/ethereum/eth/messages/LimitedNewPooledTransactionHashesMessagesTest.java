...
public class LimitedNewPooledTransactionHashesMessagesTest {
  ...
  private final List<Hash> sampleTxs = generator.transactions(1)
                                           .stream()
  ...
  private final LimitedNewPooledTransactionHashesMessages
      sampleLimitedTransactionsMessages =
          new LimitedNewPooledTransactionHashesMessages(
              sampleTransactionMessages, sampleTxs);

  @Test
  public void createLimited() {
    final List<Hash> txs = generator.transactions(6000)
                               .stream()
                               .map(Transaction::getHash)
                               .collect(Collectors.toList());
    ...
    final LimitedNewPooledTransactionHashesMessages secondMessage =
        LimitedNewPooledTransactionHashesMessages.createLimited(txs);
    assertThat(secondMessage.getIncludedTransactions().size())
        .isEqualTo(6000 - 4096);
    txs.removeAll(secondMessage.getIncludedTransactions());
    assertThat(txs.size()).isEqualTo(0);
    assertThat(firstMessage.getTransactionsMessage().getSize() +
  ...
  public void getIncludedTransactions() {
    assertThat(sampleLimitedTransactionsMessages.getIncludedTransactions())
        .isEqualTo(sampleTxs);
  }
}
