...
public class PendingTransactionsMessageSenderTest {
  ...
  public void shouldSendPendingTransactionsToEachPeer() throws Exception {
    ...
    verify(peer1).send(
        transactionsMessageContaining(transaction1, transaction2));
  ...
  public void shouldSendTransactionsInBatchesWithLimit() throws Exception {
    final Set<Hash> transactions = generator.transactions(6000)
                                       .stream()
    ...
    transactions.forEach(
        transaction
        -> transactionTracker.addToPeerSendQueue(peer1, transaction));
    ...
    final List<MessageData> sentMessages =
        messageDataArgumentCaptor.getAllValues();
    ...
    final Set<Hash> firstBatch =
        getTransactionsFromMessage(sentMessages.get(0));
    final Set<Hash> secondBatch =
    ...
    assertThat(firstBatch)
        .hasSizeBetween(expectedFirstBatchSize - toleranceDelta,
                        expectedFirstBatchSize + toleranceDelta);
    assertThat(secondBatch)
        .hasSizeBetween(expectedSecondBatchSize - toleranceDelta,
                        expectedSecondBatchSize + toleranceDelta);

    assertThat(Sets.union(firstBatch, secondBatch)).isEqualTo(transactions);
  }

  private MessageData
  transactionsMessageContaining(final Hash... transactions) {
    return argThat(message -> {
      final Set<Hash> actualSentTransactions =
