...
public class EvmToolCommand implements Runnable {
  ...
  public void run() {
    try {
      ...
      do {
        ...
        evm.runToHalt(
            messageFrame, (frame, currentGasCost, executeOperation) -> {
        ...
        if (lastLoop) {
          System.out.println(
              new JsonObject()
                  .put("gasUser", gas.minus(messageFrame.getRemainingGas())
                                      .asUInt256()
                                      .toShortHexString())
  ...
  private JsonObject createEvmTraceOperation(final MessageFrame messageFrame) {
    ...
    results.put("gasCost", messageFrame.getCurrentOperation()
                               .cost(messageFrame)
                               .asUInt256()
                               .toShortHexString());
    if (!showMemory) {
