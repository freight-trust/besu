...
public class EthNewFilterTest {
  ...
  public void newFilterWithoutFromBlockParamUsesLatestAsDefault() {
    final FilterParameter filterParameter =
        new FilterParameter(null, null, null, null, null);
    final JsonRpcRequestContext request = ethNewFilter(filterParameter);

    method.response(request);

    verify(filterManager)
        .installLogFilter(refEq(BlockParameter.LATEST), any(), any());
  }
  ...
  public void newFilterWithoutToBlockParamUsesLatestAsDefault() {
    ...
    verify(filterManager)
        .installLogFilter(any(), refEq(BlockParameter.LATEST), any());
  }
  ...
  public void newFilterWithoutAddressAndTopicsParamsInstallsEmptyLogFilter() {
    final FilterParameter filterParameter = new FilterParameter(
        BlockParameter.LATEST, BlockParameter.LATEST, null, null, null);
    final JsonRpcRequestContext request = ethNewFilter(filterParameter);
    ...
    when(filterManager.installLogFilter(any(), any(), eq(expectedLogsQuery)))
        .thenReturn("0x1");
    ...
    verify(filterManager)
        .installLogFilter(refEq(BlockParameter.LATEST),
                          refEq(BlockParameter.LATEST), eq(expectedLogsQuery));
  }

  @Test
  public void newFilterWithTopicsOnlyParamInstallsExpectedLogFilter() {
    final FilterParameter filterParameter =
        filterParamWithAddressAndTopics(null, topics());
    final JsonRpcRequestContext request = ethNewFilter(filterParameter);
    final JsonRpcResponse expectedResponse =
        new JsonRpcSuccessResponse(request.getRequest().getId(), "0x1");
  ...
  public void newFilterWithAddressOnlyParamInstallsExpectedLogFilter() {
    ...
    final FilterParameter filterParameter =
        filterParamWithAddressAndTopics(address, null);
    final JsonRpcRequestContext request = ethNewFilter(filterParameter);
    final JsonRpcResponse expectedResponse =
        new JsonRpcSuccessResponse(request.getRequest().getId(), "0x1");

    final LogsQuery expectedLogsQuery =
        new LogsQuery.Builder().address(address).build();
    when(filterManager.installLogFilter(any(), any(), eq(expectedLogsQuery)))
        .thenReturn("0x1");
  ...
  public void newFilterWithAddressAndTopicsParamInstallsExpectedLogFilter() {
    ...
    final FilterParameter filterParameter =
        filterParamWithAddressAndTopics(address, topics);
    final JsonRpcRequestContext request = ethNewFilter(filterParameter);
    final JsonRpcResponse expectedResponse =
        new JsonRpcSuccessResponse(request.getRequest().getId(), "0x1");

    final LogsQuery expectedLogsQuery = new LogsQuery.Builder()
                                            .address(address)
  ...
  public void filterWithInvalidParameters() {
    final FilterParameter invalidFilter = new FilterParameter(
        ...
        Collections.emptyList(), Hash.ZERO);
  ...
  private List<List<LogTopic>> topics() {
    return singletonList(singletonList(LogTopic.fromHexString(
        "0x000000000000000000000000a94f5374fce5edbc8e2a8697c15331677e6ebf0b")));
  }

  private FilterParameter
  filterParamWithAddressAndTopics(final Address address,
  ...
  ethNewFilter(final FilterParameter filterParameter) {
