...
public class PrivGetPrivateTransaction implements JsonRpcMethod {
  ...
  public JsonRpcResponse response(final JsonRpcRequestContext requestContext) {
    ...
    final String payloadKey = resultTransaction.getTransaction()
                                  .getPayload()
                                  .slice(0, 32)
                                  .toBase64String();
    ...
    try {
      privateTransaction = findPrivateTransactionInEnclave(
          payloadKey, resultTransaction.getTransaction().getHash(), enclaveKey,
          resultTransaction.getBlockHash().get());
    } catch (final EnclaveClientException e) {
      if (e.getMessage().equals(
              JsonRpcError.ENCLAVE_PAYLOAD_NOT_FOUND.getMessage())) {
        return new JsonRpcSuccessResponse(requestContext.getRequest().getId(),
                                          null);
      } else {
