...
public class SubscriptionBuilder {
  ...
  public Subscription build(final long subscriptionId,
    ...
    case NEW_BLOCK_HEADERS: {
      return new NewBlockHeadersSubscription(subscriptionId, connectionId,
                                             request.getIncludeTransaction());
    }
    case LOGS: {
      return new LogsSubscription(subscriptionId, connectionId,
                                  request.getFilterParameter());
    }
    case SYNCING: {
      return new SyncingSubscription(subscriptionId, connectionId,
                                     subscriptionType);
    }
    case NEW_PENDING_TRANSACTIONS:
    default:
      return new Subscription(subscriptionId, connectionId, subscriptionType,
                              request.getIncludeTransaction());
    }
  }

  @SuppressWarnings("unchecked")
  public <T> Function<Subscription, T>
  mapToSubscriptionClass(final Class<T> clazz) {
    return subscription -> {
