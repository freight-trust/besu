...
public class PrivacyQueries {
  ...
  public PrivacyQueries(final BlockchainQueries blockchainQueries,
                        final PrivateWorldStateReader privateWorldStateReader) {
    this.blockchainQueries = blockchainQueries;
    this.privateWorldStateReader = privateWorldStateReader;
  }

  public List<LogWithMetadata> matchingLogs(final String privacyGroupId,
  ...
  public List<LogWithMetadata> matchingLogs(final String privacyGroupId,
    ...
    final boolean removed =
        !blockchainQueries.blockIsOnCanonicalChain(blockHash);

    return IntStream.range(0, privateTransactionReceiptList.size())
        .mapToObj(i
                  -> LogWithMetadata.generate(
                      privateTransactionReceiptList.get(i), blockNumber,
                      blockHash,
                      privateTransactionMetadataList.get(i)
                          .getPrivacyMarkerTransactionHash(),
                      findPMTIndex(pmtHashList.get(i)), removed))
  ...
  private int findPMTIndex(final Hash pmtHash) {
    return blockchainQueries.transactionLocationByHash(pmtHash)
        .map(TransactionLocation::getTransactionIndex)
        .orElseThrow(()
                         -> new IllegalStateException(
                             "Can't find PMT index with hash " + pmtHash));
  }
}
