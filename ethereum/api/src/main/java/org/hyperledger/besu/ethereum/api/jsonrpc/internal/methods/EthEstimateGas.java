...
public class EthEstimateGas implements JsonRpcMethod {
  ...
  public EthEstimateGas(final BlockchainQueries blockchainQueries,
  ...
  private BlockHeader blockHeader() {
    ...
    return blockchainQueries.getBlockchain()
        .getBlockHeader(headBlockNumber)
        .orElse(null);
  }

  private JsonCallParameter
  overrideGasLimitAndPrice(final CallParameter callParams,
                           final long gasLimit) {
    return new JsonCallParameter(
        callParams.getFrom() != null ? callParams.getFrom().toString() : null,
        callParams.getTo() != null ? callParams.getTo().toString() : null,
        Quantity.create(gasLimit), Quantity.create(0L),
        callParams.getValue() != null ? Quantity.create(callParams.getValue())
                                      : null,
        callParams.getPayload() != null ? callParams.getPayload().toString()
                                        : null);
  }

  private Function<TransactionSimulatorResult, JsonRpcResponse>
  gasEstimateResponse(final JsonRpcRequestContext request) {
    return result
        -> result.isSuccessful()
               ? new JsonRpcSuccessResponse(
                     request.getRequest().getId(),
                     Quantity.create(result.getGasEstimate()))
  ...
  private JsonRpcErrorResponse errorResponse(
    ...
    if (validationResult != null) {
      jsonRpcError = JsonRpcErrorConverter.convertTransactionInvalidReason(
  ...
  errorResponse(final JsonRpcRequestContext request,
