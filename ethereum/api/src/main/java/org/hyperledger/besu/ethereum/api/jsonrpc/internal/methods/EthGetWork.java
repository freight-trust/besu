...
public class EthGetWork implements JsonRpcMethod {
  ...
  public EthGetWork(final MiningCoordinator miner) { this.miner = miner; }
  ...
  public JsonRpcResponse response(final JsonRpcRequestContext requestContext) {
    ...
    if (solver.isPresent()) {
      ...
      final byte[] dagSeed =
          DirectAcyclicGraphSeed.dagSeed(rawResult.getBlockNumber());
      final String[] result = {
          "0x" + BaseEncoding.base16().lowerCase().encode(
                     rawResult.getPrePowHash()),
      ...
      LOG.trace(
          "Mining is not operational, eth_getWork request cannot be processed");
      return new JsonRpcErrorResponse(requestContext.getRequest().getId(),
                                      JsonRpcError.NO_MINING_WORK_FOUND);
    }
  }
}
