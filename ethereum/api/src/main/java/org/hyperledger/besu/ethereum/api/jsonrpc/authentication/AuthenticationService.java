...
public class AuthenticationService {
  ...
  private static final JWTAuthOptionsFactory jwtAuthOptionsFactory =
      new JWTAuthOptionsFactory();

  private AuthenticationService(
      final JWTAuth jwtAuthProvider, final JWTAuthOptions jwtAuthOptions,
  ...
  /**
   * Creates a ready for use set of authentication providers if authentication
   * is configured to be on
   *
   * @param vertx The vertx instance that will be providing requests that this
   *     set of authentication providers will be handling
   * @param config The {{@link JsonRpcConfiguration}} that describes this rpc
   *     setup
   * @return Optionally an authentication service. If empty then authentication
   *     isn't to be enabled on this service
   */
  public static Optional<AuthenticationService>
  create(final Vertx vertx, final JsonRpcConfiguration config) {
   ...
   */
  public static Optional<AuthenticationService>
  create(final Vertx vertx, final WebSocketConfiguration config) {
    return create(vertx, config.isAuthenticationEnabled(),
                  config.getAuthenticationCredentialsFile(),
                  config.getAuthenticationPublicKeyFile());
  }

  private static Optional<AuthenticationService>
  create(final Vertx vertx, final boolean authenticationEnabled,
    ...
    final JWTAuthOptions jwtAuthOptions =
        authenticationPublicKeyFile == null
            ...
            : jwtAuthOptionsFactory.createForExternalPublicKey(
                  authenticationPublicKeyFile);

    final Optional<AuthProvider> credentialAuthProvider =
        makeCredentialAuthProvider(vertx, authenticationEnabled,
                                   authenticationCredentialsFile);

    return Optional.of(
        new AuthenticationService(JWTAuth.create(vertx, jwtAuthOptions),
                                  jwtAuthOptions, credentialAuthProvider));
  }

  private static Optional<AuthProvider> makeCredentialAuthProvider(
      final Vertx vertx, final boolean authenticationEnabled,
      @Nullable final String authenticationCredentialsFile) {
    if (authenticationEnabled && authenticationCredentialsFile != null) {
      return Optional.of(new TomlAuthOptions()
                             .setTomlPath(authenticationCredentialsFile)
  ...
  /**
   * Handles a login request and checks the provided credentials against our
  ...
  private void login(final RoutingContext routingContext,
    ...
    credentialAuthProvider.authenticate(authParams, (r) -> {
      if (r.failed()) {
        routingContext.response()
        ...
        final String privacyPublicKey =
            user.principal().getString("privacyPublicKey");
        if (privacyPublicKey != null) {
          jwtContents.put("privacyPublicKey", privacyPublicKey);
        }

        final String token =
            jwtAuthProvider.generateToken(jwtContents, options);
  ...
  public JWTAuth getJwtAuthProvider() { return jwtAuthProvider; }
}
