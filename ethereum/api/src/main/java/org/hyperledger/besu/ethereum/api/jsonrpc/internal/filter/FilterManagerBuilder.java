...
public class FilterManagerBuilder {
  ...
  filterIdGenerator(final FilterIdGenerator filterIdGenerator) {
    this.filterIdGenerator = filterIdGenerator;
    return this;
  }

  public FilterManagerBuilder
  filterRepository(final FilterRepository filterRepository) {
    this.filterRepository = filterRepository;
    return this;
  }

  public FilterManagerBuilder
  blockchainQueries(final BlockchainQueries blockchainQueries) {
    this.blockchainQueries = blockchainQueries;
    return this;
  }

  public FilterManagerBuilder
  transactionPool(final TransactionPool transactionPool) {
  ...
  public FilterManager build() {
    if (blockchainQueries == null) {
      throw new IllegalStateException(
          "BlockchainQueries is required to build FilterManager");
    }

    if (transactionPool == null) {
      throw new IllegalStateException(
          "TransactionPool is required to build FilterManager");
    }

    if (privacyQueries.isEmpty() && privacyParameters.isPresent() &&
        privacyParameters.get().isEnabled()) {
      privacyQueries = Optional.of(new PrivacyQueries(
          blockchainQueries,
          privacyParameters.get().getPrivateWorldStateReader()));
    }

    return new FilterManager(blockchainQueries, transactionPool, privacyQueries,
