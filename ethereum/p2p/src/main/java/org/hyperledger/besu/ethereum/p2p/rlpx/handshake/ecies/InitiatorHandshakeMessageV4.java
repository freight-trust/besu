...
public final class InitiatorHandshakeMessageV4
  ...
  public static InitiatorHandshakeMessageV4
  create(final SECP256K1.PublicKey ourPubKey,
    ...
    return new InitiatorHandshakeMessageV4(
        ourPubKey, SECP256K1.sign(staticSharedSecret.xor(nonce), ephKeyPair),
        ephKeyPair.getPublicKey(), nonce);
  }
   ...
   * @param nodeKey Abstraction of the local nodes keys and associated
   *     cryptographic operations
   * @return The decoded message.
   */
  public static InitiatorHandshakeMessageV4 decode(final Bytes bytes,
                                                   final NodeKey nodeKey) {
    final RLPInput input = new BytesValueRLPInput(bytes, true);
    input.enterList();
    final SECP256K1.Signature signature =
        SECP256K1.Signature.decode(input.readBytes());
    final SECP256K1.PublicKey pubKey =
        SECP256K1.PublicKey.create(input.readBytes());
  ...
  private InitiatorHandshakeMessageV4(final SECP256K1.PublicKey pubKey,
                                      final SECP256K1.Signature signature,
                                      final SECP256K1.PublicKey ephPubKey,
                                      final Bytes32 nonce) {
    this.pubKey = pubKey;
