...
final class HandshakeHandlerOutbound extends AbstractHandshakeHandler {
  ...
  public HandshakeHandlerOutbound(
      final NodeKey nodeKey, final Peer peer,
      final List<SubProtocol> subProtocols, final LocalNode localNode,
      final CompletableFuture<PeerConnection> connectionFuture,
      final PeerConnectionEventDispatcher connectionEventDispatcher,
      final MetricsSystem metricsSystem) {
    super(subProtocols, localNode, Optional.of(peer), connectionFuture,
          connectionEventDispatcher, metricsSystem);
    handshaker.prepareInitiator(nodeKey,
                                SECP256K1.PublicKey.create(peer.getId()));
  ...
  public void channelActive(final ChannelHandlerContext ctx) throws Exception {
    ...
    ctx.writeAndFlush(first).addListener(f -> {
      if (f.isSuccess()) {
        LOG.debug("Wrote initial crypto handshake message to {}.",
