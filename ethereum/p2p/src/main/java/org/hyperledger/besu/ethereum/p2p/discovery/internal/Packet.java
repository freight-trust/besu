...
public class Packet {
  ...
  private Packet(final PacketType type, final PacketData data,
    ...
    this.hash = keccak256(
        Bytes.concatenate(encodeSignature(signature), typeBytes, dataBytes));
    this.publicKey = nodeKey.getPublicKey();
  }

  private Packet(final PacketType packetType, final PacketData packetData,
                 final Bytes message) {
    ...
    final Bytes rest =
        message.slice(SIGNATURE_INDEX, message.size() - SIGNATURE_INDEX);
  ...
  public static Packet create(final PacketType packetType,
  ...
  public static Packet decode(final Buffer message) {
    checkGuard(message.length() >= PACKET_DATA_INDEX,
               PeerDiscoveryPacketDecodingException::new,
               "Packet too short: expected at least %s bytes, got %s",
               PACKET_DATA_INDEX, message.length());

    final byte type = message.getByte(PACKET_TYPE_INDEX);

    final PacketType packetType = PacketType.forByte(type).orElseThrow(
        ()
            -> new PeerDiscoveryPacketDecodingException(
                "Unrecognized packet type: " + type));

    final PacketType.Deserializer<?> deserializer =
        packetType.getDeserializer();
    final PacketData packetData;
    try {
      packetData =
          deserializer.deserialize(RLP.input(message, PACKET_DATA_INDEX));
    } catch (final RLPException e) {
      throw new PeerDiscoveryPacketDecodingException(
          "Malformed packet of type: " + packetType, e);
    }
  ...
  public Buffer encode() {
    ...
    final Buffer buffer = Buffer.buffer(hash.size() + encodedSignature.size() +
                                        1 + encodedData.encodedSize());
  ...
  protected void appendEncoded(final BytesValueRLPOutput encoded,
                               final Buffer buffer) {
    ...
    // We want to append to the buffer, and Buffer always grows to accommodate
    // anything writing, so we write the last byte we know we'll need to make it
    // resize accordingly.
  ...
  public <T extends PacketData> Optional<T>
  getPacketData(final Class<T> expectedPacketType) {
  ...
  public Bytes getNodeId() { return publicKey.getEncodedBytes(); }
  ...
  private static SECP256K1.Signature
  decodeSignature(final Bytes encodedSignature) {
    checkArgument(encodedSignature != null && encodedSignature.size() == 65,
                  "encodedSignature is 65 bytes");
