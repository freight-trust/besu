...
final class EncryptedMessage {
   ...
   * @param nodeKey Abstraction of this nodes private key & associated
  ...
  public static Bytes decryptMsg(final Bytes msgBytes, final NodeKey nodeKey)
    ...
    // Extract the ephemeral public key, stripping off the first byte (0x04),
    // which designates it's an uncompressed key.
    final SECP256K1.PublicKey ephPubKey =
        SECP256K1.PublicKey.create(msgBytes.slice(1, 64));
   ...
   * @throws InvalidCipherTextException Thrown if decryption failed.
   */
  public static Bytes decryptMsgEIP8(final Bytes msgBytes,
                                     final NodeKey nodeKey)
      throws InvalidCipherTextException {
    final SECP256K1.PublicKey ephPubKey =
        SECP256K1.PublicKey.create(msgBytes.slice(3, 64));
   ...
   */
  public static Bytes encryptMsg(final Bytes bytes,
                                 final SECP256K1.PublicKey remoteKey)
      throws InvalidCipherTextException {
    // TODO: check size.
    final ECIESEncryptionEngine engine =
        ECIESEncryptionEngine.forEncryption(remoteKey);
    ...
    // Create the output message by concatenating the ephemeral public key
    // (prefixed with 0x04 to designate uncompressed), IV, and encrypted bytes.
    final MutableBytes answer = MutableBytes.create(
        1 + ECIESHandshaker.PUBKEY_LENGTH + IV_SIZE + encrypted.size());
   ...
   * @throws InvalidCipherTextException Thrown if encryption failed.
   */
  public static Bytes encryptMsgEip8(final Bytes message,
                                     final SECP256K1.PublicKey remoteKey)
      throws InvalidCipherTextException {
    final ECIESEncryptionEngine engine =
        ECIESEncryptionEngine.forEncryption(remoteKey);
    ...
    // (prefixed with 0x04 to designate uncompressed), IV, and encrypted bytes.
    final MutableBytes answer = MutableBytes.create(
        3 + ECIESHandshaker.PUBKEY_LENGTH + IV_SIZE + encrypted.size());
  ...
  private static Bytes addPadding(final Bytes message) {
    ...
    return Bytes.wrap(ByteBuffer.allocate(raw.length + padding)
                          .put(raw)
                          .put(paddingBytes)
                          .array());
  }
