...
public class ECIESHandshakeTest {
  ...
  private static class Input {
    ...
    private static final KeyPair initiatorKeyPair =
        KeyPair.create(PrivateKey.create(h32(
            "0x5e173f6ac3c669587538e7727cf19b782a4f2fda07c1eaa662c593e5e85e3051")));
    private static final KeyPair initiatorEphKeyPair =
        KeyPair.create(PrivateKey.create(h32(
            "0x19c2185f4f40634926ebed3af09070ca9e029f2edd5fae6253074896205f5f6c")));
    private static final KeyPair responderKeyPair =
        KeyPair.create(PrivateKey.create(h32(
            "0xc45f950382d542169ea207959ee0220ec1491755abe405cd7498d6b16adb6df8")));
    private static final KeyPair responderEphKeyPair =
        KeyPair.create(PrivateKey.create(h32(
            "0xd25688cf0ab10afa1a0e2dba7853ed5f1e5bf1c631757ed4e103b593ff3f5620")));

    // Nonces.
    private static final Bytes32 initiatorNonce = h32(
        "0xcd26fecb93657d1cd9e9eaf4f8be720b56dd1d39f190c4e1c6b7ec66f077bb11");
    private static final Bytes32 responderNonce = h32(
        "0xf37ec61d84cea03dcc5e8385db93248584e8af4b4d1c832d8c7453c0089687a7");
  ...
  private static class Messages {
    ...
    // Encrypted messages -- we cannot assert against these because the
    // Integrated Encryption Scheme is designed to use a different ephemeral key
    // everytime.
  ...
  private static class Expectations {
    ...
    private static final byte[] aesSecret =
        h32("0xc0458fa97a5230830e05f4f20b7c755c1d4e54b1ce5cf43260bb191eef4e418d")
            .toArray();
    private static final byte[] macSecret =
        h32("0x48c938884d5067a1598272fcddaa4b833cd5e7d92e8228c0ecdfabbe68aef7f1")
            .toArray();
    private static final byte[] token =
        h32("0x3f9ec2592d1554852b1f54d228f042ed0a9310ea86d038dc2b401ba8cd7fdac4")
            .toArray();
    private static final byte[] initialEgressMac =
        h32("0x09771e93b1a6109e97074cbe2d2b0cf3d3878efafe68f53c41bb60c0ec49097e")
            .toArray();
    private static final byte[] initialIngressMac =
        h32("0x75823d96e23136c89666ee025fb21a432be906512b3dd4a3049e898adb433847")
            .toArray();
  }
  ...
  public void authPlainTextWithEncryption() {
    ...
    initiator.prepareInitiator(NodeKeyUtils.createFrom(Input.initiatorKeyPair),
                               Input.responderKeyPair.getPublicKey());
    ...
    // Give the responder the initiator's request. Check that it has a message
    // to send.
    final ByteBuf responderRp =
        responder.handleMessage(initiatorRq)
            .orElseThrow(
                () -> new AssertionFailedError("Expected responder message"));
    assertThat(responder.getPartyEphPubKey())
        .isEqualTo(initiator.getEphKeyPair().getPublicKey());
    assertThat(responder.getInitiatorNonce())
        .isEqualTo(initiator.getInitiatorNonce());
    assertThat(responder.partyPubKey())
        .isEqualTo(initiator.getNodeKey().getPublicKey());
    ...
    assertThat(initiator.getPartyEphPubKey())
        .isEqualTo(responder.getEphKeyPair().getPublicKey());
  ...
  public void encryptedInitiationRqFromPyEvm() {
    ...
    final Optional<ByteBuf> responderMsg = responder.handleMessage(
        Unpooled.wrappedBuffer(Input.pyEvmInitiatorRqEnc));
  ...
  public void ingressEgressMacsAsExpected() {
    ...
    // Override the message sent from the responder with the test vector, and
    ...
    assertThat(initiator.secrets().getAesSecret())
        .isEqualTo(Expectations.aesSecret);
    assertThat(initiator.secrets().getMacSecret())
        .isEqualTo(Expectations.macSecret);
    assertThat(initiator.secrets().getToken()).isEqualTo(Expectations.token);
    assertThat(initiator.secrets().getIngressMac())
        .isEqualTo(Expectations.initialIngressMac);
    assertThat(initiator.secrets().getEgressMac())
        .isEqualTo(Expectations.initialEgressMac);

    // Assert that the responder's secrets match the expected values, where the
    // ingress and egress are reversed.
    assertThat(responder.secrets().getAesSecret())
        .isEqualTo(Expectations.aesSecret);
    assertThat(responder.secrets().getMacSecret())
        .isEqualTo(Expectations.macSecret);
    assertThat(responder.secrets().getToken()).isEqualTo(Expectations.token);
    assertThat(responder.secrets().getIngressMac())
        .isEqualTo(Expectations.initialEgressMac);
    assertThat(responder.secrets().getEgressMac())
        .isEqualTo(Expectations.initialIngressMac);
  }

  private static Bytes h(final String hex) { return Bytes.fromHexString(hex); }
