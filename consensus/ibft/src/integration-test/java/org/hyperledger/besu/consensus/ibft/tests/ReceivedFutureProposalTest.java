...
/**
 * Ensure the Ibft component responds appropriately when a future round Proposal
 * message is received.
 */
public class ReceivedFutureProposalTest {
  ...
  public void proposalWithEmptyPrepareCertificatesOfferNewBlock() {
    ...
    final Block blockToPropose = context.createBlockForProposalFromChainHead(
        nextRoundId.getRoundNumber(), 15);
    ...
    final ValidatorPeer nextProposer =
        context.roundSpecificPeers(nextRoundId).getProposer();
    ...
    final Prepare expectedPrepare = localNodeMessageFactory.createPrepare(
        targetRound, blockToPropose.getHash());
  ...
  @Test
  public void
  proposalFromIllegalSenderIsDiscardedAndNoPrepareForNewRoundIsSent() {
    final ConsensusRoundIdentifier nextRoundId =
        new ConsensusRoundIdentifier(1, 1);
    final Block blockToPropose = context.createBlockForProposalFromChainHead(
        nextRoundId.getRoundNumber(), 15);
  ...
  @Test
  public void
  proposalWithPrepareCertificateResultsInNewRoundStartingWithExpectedBlock() {
    ...
    peers.verifyMessagesReceived(localNodeMessageFactory.createPrepare(
        nextRoundId, reproposedBlock.getHash()));
  }

  @Test
  public void proposalMessageForPriorRoundIsNotActioned() {
    // first move to a future round, then inject a proposal for a prior round,
    // local node should send no messages.
    final ConsensusRoundIdentifier futureRound =
        new ConsensusRoundIdentifier(1, 2);
    peers.roundChange(futureRound);

    final ConsensusRoundIdentifier interimRound =
        new ConsensusRoundIdentifier(1, 1);
    ...
    interimRoundProposer.injectProposalForFutureRound(
        interimRound, new RoundChangeCertificate(roundChangePayloads),
  ...
  @Test
  public void
  receiveRoundStateIsNotLostIfASecondProposalMessageIsReceivedForCurrentRound() {
    final Block initialBlock =
        context.createBlockForProposalFromChainHead(0, 15);
    final Block reproposedBlock =
        context.createBlockForProposalFromChainHead(1, 15);
    ...
    final RoundSpecificPeers nextRoles =
        context.roundSpecificPeers(nextRoundId);
    ...
    // Inject a prepare, then re-inject the proposal - then ensure only a single
    // prepare is enough to trigger a Commit transmission from the local node
    nextRoles.getNonProposing(0).injectPrepare(nextRoundId,
                                               reproposedBlock.getHash());
    ...
    nextRoles.getNonProposing(1).injectPrepare(nextRoundId,
                                               reproposedBlock.getHash());

    final Commit expectedCommit =
        new Commit(IntegrationTestHelpers.createSignedCommitPayload(
            nextRoundId, reproposedBlock,
