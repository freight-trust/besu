...
public class FutureHeightTest {
  ...
  private final ConsensusRoundIdentifier futureHeightRoundId =
      new ConsensusRoundIdentifier(2, 0);
  ...
  public void messagesForFutureHeightAreBufferedUntilChainHeightCatchesUp() {
    ...
    final Block signedCurrentHeightBlock = IbftHelpers.createSealedBlock(
        currentHeightBlock, peers.sign(currentHeightBlock.getHash()));

    final Block futureHeightBlock = context.createBlockForProposal(
        signedCurrentHeightBlock.getHeader(), 0, 60);
    ...
    peers.getProposer().verifyEstimatedChainHeightEquals(
        futureHeightBlock.getHeader().getNumber() - 1);

    // Inject prepares and commits from all peers
    peers.prepareForNonProposing(futureHeightRoundId,
                                 futureHeightBlock.getHash());
    peers.forNonProposing(peer
                          -> peer.verifyEstimatedChainHeightEquals(
                              futureHeightBlock.getHeader().getNumber() - 1));

    peers.commitForNonProposing(futureHeightRoundId,
                                futureHeightBlock.getHash());
    ...
    final Prepare expectedPrepareMessage =
        localNodeMessageFactory.createPrepare(futureHeightRoundId,
                                              futureHeightBlock.getHash());

    final Commit expectedCommitMessage = new Commit(
        createSignedCommitPayload(futureHeightRoundId, futureHeightBlock,
  ...
  public void messagesFromPreviousHeightAreDiscarded() {
    ...
    peers.getNonProposing(0).injectPrepare(roundId,
                                           currentHeightBlock.getHash());

    final Prepare expectedPrepareMessage =
        localNodeMessageFactory.createPrepare(roundId,
                                              currentHeightBlock.getHash());
    ...
    // Inject prepares and commits from all peers for the 'previous' round (i.e.
    // the height from before the block arrived).
  ...
  public void multipleNewChainHeadEventsDoesNotRestartCurrentHeightManager() {
    ...
    context.getController().handleNewBlockEvent(
        new NewChainHead(context.getBlockchain().getChainHeadHeader()));

    // Should only require 1 more prepare to close it out
    peers.getNonProposing(1).injectPrepare(roundId,
                                           currentHeightBlock.getHash());

    final Commit expectedCommitMessage = new Commit(
        createSignedCommitPayload(roundId, currentHeightBlock,
                                  context.getLocalNodeParams().getNodeKey()));
  ...
  public void correctMessagesAreExtractedFromFutureHeightBuffer() {
    ...
    final Block nextHeightBlock = context.createBlockForProposal(
        signedCurrentHeightBlock.getHeader(), 0, 60);
    final Block signedNextHeightBlock = IbftHelpers.createSealedBlock(
        nextHeightBlock, peers.sign(nextHeightBlock.getHash()));

    final Block futureHeightBlock = context.createBlockForProposal(
        signedNextHeightBlock.getHeader(), 0, 90);

    final ConsensusRoundIdentifier nextHeightRoundId =
        new ConsensusRoundIdentifier(2, 0);
    final ConsensusRoundIdentifier futureHeightRoundId =
        new ConsensusRoundIdentifier(3, 0);

    // Inject prepares and commits from all peers into FutureHeight (2 height
    // time)
    peers.prepareForNonProposing(futureHeightRoundId,
                                 futureHeightBlock.getHash());
    peers.commitForNonProposing(futureHeightRoundId,
    ...
    final Prepare expectedPrepareMessage =
        localNodeMessageFactory.createPrepare(nextHeightRoundId,
                                              nextHeightBlock.getHash());

    // Assert ONLY a prepare message was received, not any commits (i.e.
    // futureHeightRoundId messages have not been used.
    peers.verifyMessagesReceived(expectedPrepareMessage);
    ...
    assertThat(context.getCurrentChainHeight()).isEqualTo(2);
    context.getController().handleNewBlockEvent(
        new NewChainHead(signedNextHeightBlock.getHeader()));
