...
public class FutureRoundTest {
  ...
  private final ConsensusRoundIdentifier futureRoundId =
      new ConsensusRoundIdentifier(1, 5);
  private final RoundSpecificPeers futurePeers =
      context.roundSpecificPeers(futureRoundId);
  ...
  public void messagesForFutureRoundAreNotActionedUntilRoundIsActive() {
    final Block futureBlock = context.createBlockForProposalFromChainHead(
        futureRoundId.getRoundNumber(), 60);
    final int quorum =
        IbftHelpers.calculateRequiredValidatorQuorum(NETWORK_SIZE);
    final ConsensusRoundIdentifier subsequentRoundId =
        new ConsensusRoundIdentifier(1, 6);
    final RoundSpecificPeers subsequentRoles =
        context.roundSpecificPeers(subsequentRoundId);
    ...
    for (int i = 0; i < quorum - 3; i++) {
      futurePeers.getNonProposing(i).injectPrepare(futureRoundId,
                                                   futureBlock.getHash());
    }

    for (int i = 0; i < quorum - 2; i++) {
      futurePeers.getNonProposing(i).injectCommit(futureRoundId,
                                                  futureBlock.getHash());
    }

    // inject a prepare and a commit from a subsequent round, and ensure no
    // transmissions are created
    subsequentRoles.getNonProposing(1).injectPrepare(subsequentRoundId,
                                                     futureBlock.getHash());
    subsequentRoles.getNonProposing(1).injectCommit(subsequentRoundId,
                                                    futureBlock.getHash());

    peers.verifyNoMessagesReceived();
    assertThat(context.getBlockchain().getChainHeadBlockNumber()).isEqualTo(0);

    // inject a newRound to move to 'futureRoundId', and ensure localnode sends
    // prepare, commit and updates blockchain
    futurePeers.getProposer().injectProposalForFutureRound(
        futureRoundId,
        new RoundChangeCertificate(
            futurePeers.createSignedRoundChangePayload(futureRoundId)),
        futureBlock);

    final Prepare expectedPrepare = localNodeMessageFactory.createPrepare(
        futureRoundId, futureBlock.getHash());

    peers.verifyMessagesReceived(expectedPrepare);

    // following 1 more prepare, a commit msg will be sent
    futurePeers.getNonProposing(quorum - 3)
        .injectPrepare(futureRoundId, futureBlock.getHash());

    final Commit expectedCommit = localNodeMessageFactory.createCommit(
        futureRoundId, futureBlock.getHash(),
    ...
    futurePeers.getNonProposing(quorum - 2)
        .injectCommit(futureRoundId, futureBlock.getHash());
  ...
  public void priorRoundsCannotBeCompletedAfterReceptionOfNewRound() {
    final Block initialBlock = context.createBlockForProposalFromChainHead(
    ...
    // At this stage, the local node has 2 commit msgs (proposer and local) so
    // has not committed
    assertThat(context.getBlockchain().getChainHeadBlockNumber()).isEqualTo(0);
    ...
    final Prepare expectedFuturePrepare = localNodeMessageFactory.createPrepare(
        futureRoundId, futureBlock.getHash());
