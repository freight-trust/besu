...
/**
 * These tests assume the basic function of the Ibft Round State Machine has
 * been proven via the LocalNodeIsNotProposerTest.
 */
public class LocalNodeIsProposerTest {
  ...
  // Configuration ensures unit under test will be responsible for sending first
  // proposal
  ...
  public void setup() {
    expectedProposedBlock =
        context.createBlockForProposalFromChainHead(0, blockTimeStamp);
    expectedTxProposal = localNodeMessageFactory.createProposal(
        roundId, expectedProposedBlock, Optional.empty());

    expectedTxCommit = new Commit(
        createSignedCommitPayload(roundId, expectedProposedBlock,
                                  context.getLocalNodeParams().getNodeKey()));

    // Trigger "block timer" to send proposal.
    context.getController().handleBlockTimerExpiry(
        new BlockTimerExpiry(roundId));
  }
  ...
  public void basicCase() {
    ...
    peers.getNonProposing(1).injectCommit(roundId,
                                          expectedProposedBlock.getHash());
    assertThat(context.getBlockchain().getChainHeadBlockNumber()).isEqualTo(0);
    peers.verifyNoMessagesReceived();

    peers.getNonProposing(2).injectCommit(roundId,
                                          expectedProposedBlock.getHash());
