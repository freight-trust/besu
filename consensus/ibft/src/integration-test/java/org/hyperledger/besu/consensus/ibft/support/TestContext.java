...
/*
Responsible for creating an environment in which integration testing can be
conducted.

The test setup is an 'n' node network, one of which is the local node (i.e. the
Unit Under Test).

There is some complexity with determining the which node is the proposer etc.
THus necessitating NetworkLayout and RoundSpecificNodeRoles concepts.
 */
public class TestContext {
  ...
  public TestContext(final Map<Address, ValidatorPeer> remotePeers,
  ...
  public Block createBlockForProposal(final BlockHeader parent, final int round,
                                      final long timestamp) {
    return finalState.getBlockCreatorFactory()
        .create(parent, round)
        .createBlock(timestamp);
  }

  public Block createBlockForProposalFromChainHead(final int round,
                                                   final long timestamp) {
    return createBlockForProposal(blockchain.getChainHeadHeader(), round,
                                  timestamp);
  }

  public RoundSpecificPeers
  roundSpecificPeers(final ConsensusRoundIdentifier roundId) {
    // This will return NULL if the LOCAL node is the proposer for the specified
    // round
    final Address proposerAddress = finalState.getProposerForRound(roundId);
    final ValidatorPeer proposer =
        remotePeers.getOrDefault(proposerAddress, null);

    final List<ValidatorPeer> nonProposers =
        new ArrayList<>(remotePeers.values());
  ...
  public NodeParams getLocalNodeParams() {
    return new NodeParams(finalState.getLocalAddress(),
