...
public class IbftMiningCoordinatorTest {
  ...
  @Before
  public void setup() {
    ibftMiningCoordinator = new IbftMiningCoordinator(
        ibftExecutors, controller, ibftProcessor, ibftBlockCreatorFactory,
        blockChain, eventQueue);
  ...
  public void getsMinTransactionGasPrice() {
    ...
    when(ibftBlockCreatorFactory.getMinTransactionGasPrice())
        .thenReturn(minGasPrice);
    assertThat(ibftMiningCoordinator.getMinTransactionGasPrice())
        .isEqualTo(minGasPrice);
  }
  ...
  @Test
  public void addsNewChainHeadEventWhenNewCanonicalHeadBlockEventReceived()
      throws Exception {
    BlockAddedEvent headAdvancement = BlockAddedEvent.createForHeadAdvancement(
    ...
    final NewChainHead ibftEvent =
        (NewChainHead)eventQueue.poll(1, TimeUnit.SECONDS);
