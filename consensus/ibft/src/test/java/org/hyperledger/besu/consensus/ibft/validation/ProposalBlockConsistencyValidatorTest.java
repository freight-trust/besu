...
public class ProposalBlockConsistencyValidatorTest {
  ...
  public void blockDigestMisMatchWithMessageRoundFails() {
    ...
    final Block misMatchedBlock = IbftBlockInterface.replaceRoundInBlock(
        block, roundIdentifier.getRoundNumber() + 1,
        IbftBlockHeaderFunctions.forCommittedSeal());

    assertThat(consistencyChecker.validateProposalMatchesBlock(
  ...
  public void blockDigestMatchesButRoundDiffersFails() {
    ...
    assertThat(consistencyChecker.validateProposalMatchesBlock(
                   proposalMsg.getSignedPayload(), block))
        .isFalse();
  }
  ...
  public void blockWithMismatchedNumberFails() {
    ...
    final Proposal proposalMsg = proposerMessageFactory.createProposal(
        futureHeight, block, Optional.empty());
