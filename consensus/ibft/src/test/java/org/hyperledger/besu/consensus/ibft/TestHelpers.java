...
public class TestHelpers {
  ...
  public static ConsensusRoundIdentifier
  createFrom(final ConsensusRoundIdentifier initial, final int offSetSequence,
             final int offSetRound) {
    return new ConsensusRoundIdentifier(initial.getSequenceNumber() +
                                            offSetSequence,
                                        initial.getRoundNumber() + offSetRound);
  }

  public static Block
  createProposalBlock(final List<Address> validators,
                      final ConsensusRoundIdentifier roundId) {
    final Bytes extraData =
        new IbftExtraData(Bytes.wrap(new byte[32]), Collections.emptyList(),
                          Optional.empty(), roundId.getRoundNumber(),
                          validators)
            .encode();
  ...
  createSignedProposalPayload(final NodeKey signerNodeKey) {
    return createSignedProposalPayloadWithRound(signerNodeKey, 0xFEDCBA98);
  }

  public static Proposal
  createSignedProposalPayloadWithRound(final NodeKey signerNodeKey,
                                       final int round) {
  ...
  public static Prepare createSignedPreparePayload(final NodeKey signerKeys) {
    ...
    return messageFactory.createPrepare(roundIdentifier,
  ...
  public static RoundChange
  createSignedRoundChangePayload(final NodeKey signerKeys) {
    final MessageFactory messageFactory = new MessageFactory(signerKeys);
    final ConsensusRoundIdentifier roundIdentifier =
        new ConsensusRoundIdentifier(0x1234567890ABCDEFL, 0xFEDCBA98);
