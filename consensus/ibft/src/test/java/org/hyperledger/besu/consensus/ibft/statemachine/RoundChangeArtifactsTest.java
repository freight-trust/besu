...
public class RoundChangeArtifactsTest {
  ...
  private final ConsensusRoundIdentifier targetRound =
      new ConsensusRoundIdentifier(chainHeight, 5);
  ...
  private PreparedRoundArtifacts
  createPreparedRoundArtefacts(final int fromRound) {

    final ConsensusRoundIdentifier preparedRound =
        new ConsensusRoundIdentifier(chainHeight, fromRound);
    final Block block =
        TestHelpers.createProposalBlock(emptyList(), preparedRound);
    ...
    return new PreparedRoundArtifacts(
        ...
        messageFactories.stream()
            .map(factory
                 -> factory.createPrepare(preparedRound, block.getHash()))
            .collect(Collectors.toList()));
  }

  private RoundChange
  createRoundChange(final int fromRound,
                    final boolean containsPrepareCertificate) {
    if (containsPrepareCertificate) {
      return messageFactories.get(0).createRoundChange(
          targetRound, Optional.of(createPreparedRoundArtefacts(fromRound)));
    } else {
  ...
  public void noRoundChangesPreparedThereforeReportedBlockIsEmpty() {
    final List<RoundChange> roundChanges = Lists.newArrayList(
        createRoundChange(1, false), createRoundChange(2, false),
        createRoundChange(3, false));

    final RoundChangeArtifacts artifacts =
        RoundChangeArtifacts.create(roundChanges);
