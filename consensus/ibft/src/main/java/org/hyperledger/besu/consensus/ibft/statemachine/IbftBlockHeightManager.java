 ...
 * One of these is created when a new block is imported to the chain. It
 * immediately then creates a Round-0 object, and sends a Proposal message. If
 * the round times out prior to importing a block, this class is responsible for
 * creating a RoundChange message and transmitting it.
 */
public class IbftBlockHeightManager implements BlockHeightManager {
  ...
  private final Map<Integer, RoundState> futureRoundStateBuffer =
      Maps.newHashMap();
  private final FutureRoundProposalMessageValidator
      futureRoundProposalMessageValidator;
  private final Clock clock;
  private final Function<ConsensusRoundIdentifier, RoundState>
      roundStateCreator;
  private final IbftFinalState finalState;

  private Optional<PreparedRoundArtifacts> latestPreparedRoundArtifacts =
      Optional.empty();

  private IbftRound currentRound;

  public IbftBlockHeightManager(
      final BlockHeader parentHeader, final IbftFinalState finalState,
      final RoundChangeManager roundChangeManager,
      final IbftRoundFactory ibftRoundFactory, final Clock clock,
    ...
    roundStateCreator = (roundIdentifier)
    ...
    if (finalState.isLocalNodeProposerForRound(
  ...
  @Override
  public void
  handleBlockTimerExpiry(final ConsensusRoundIdentifier roundIdentifier) {
    if (roundIdentifier.equals(currentRound.getRoundIdentifier())) {
      currentRound.createAndSendProposalMessage(clock.millis() / 1000);
    } else {
      LOG.trace("Block timer expired for a round ({}) other than current ({})",
                roundIdentifier, currentRound.getRoundIdentifier());
    }
  }
  ...
  public void roundExpired(final RoundExpiry expire) {
    if (!expire.getView().equals(currentRound.getRoundIdentifier())) {
      LOG.trace(
          ...
          currentRound.getRoundIdentifier(), expire.getView());
      return;
    }
    ...
    try {
      final RoundChange localRoundChange = messageFactory.createRoundChange(
          currentRound.getRoundIdentifier(), latestPreparedRoundArtifacts);

      // Its possible the locally created RoundChange triggers the transmission
      // of a NewRound message - so it must be handled accordingly.
  ...
  public void handleProposalPayload(final Proposal proposal) {
    ...
    if (messageAge == PRIOR_ROUND) {
      LOG.trace("Received Proposal Payload for a prior round={}",
                proposal.getRoundIdentifier());
    } else {
      if (messageAge == FUTURE_ROUND) {
        if (!futureRoundProposalMessageValidator.validateProposalMessage(
                proposal)) {
          LOG.info(
              "Received future Proposal which is illegal, no round change triggered.");
          return;
        }
  ...
  public void handlePreparePayload(final Prepare prepare) {
    ...
    actionOrBufferMessage(prepare, currentRound::handlePrepareMessage,
  ...
  public void handleCommitPayload(final Commit commit) {
    ...
    actionOrBufferMessage(commit, currentRound::handleCommitMessage,
                          RoundState::addCommitMessage);
  }

  private <P extends Payload, M extends IbftMessage<P>> void
  actionOrBufferMessage(final M ibftMessage, final Consumer<M> inRoundHandler,
                        final BiConsumer<RoundState, M> buffer) {
    final MessageAge messageAge = determineAgeOfPayload(
        ibftMessage.getRoundIdentifier().getRoundNumber());
    if (messageAge == CURRENT_ROUND) {
      ...
      final ConsensusRoundIdentifier msgRoundId =
          ibftMessage.getRoundIdentifier();
      final RoundState roundstate = futureRoundStateBuffer.computeIfAbsent(
          msgRoundId.getRoundNumber(),
          k -> roundStateCreator.apply(msgRoundId));
  ...
  public void handleRoundChangePayload(final RoundChange message) {
    ...
    if (messageAge == PRIOR_ROUND) {
      LOG.trace(
          "Received RoundChange Payload for a prior round. targetRound={}",
          targetRound);
      return;
    }
  ...
  private void startNewRound(final int roundNumber) {
    ...
    if (futureRoundStateBuffer.containsKey(roundNumber)) {
      currentRound = roundFactory.createNewRoundWithState(
  ...
  private MessageAge determineAgeOfPayload(final int messageRoundNumber) {
    final int currentRoundNumber =
        currentRound.getRoundIdentifier().getRoundNumber();
  ...
  public enum MessageAge { PRIOR_ROUND, CURRENT_ROUND, FUTURE_ROUND }
}
