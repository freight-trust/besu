...
public class IbftQueryServiceImpl
    extends PoaQueryServiceImpl implements IbftQueryService {

  public IbftQueryServiceImpl(final BlockInterface blockInterface,
  ...
  getSignersFrom(final org.hyperledger.besu.plugin.data.BlockHeader header) {
    final BlockHeader headerFromChain = getHeaderFromChain(header);
    final IbftExtraData extraData = IbftExtraData.decode(headerFromChain);
  ...
  private BlockHeader getHeaderFromChain(
    ...
    final Hash blockHash =
        Hash.wrap(Bytes32.wrap(header.getBlockHash().toArray()));
