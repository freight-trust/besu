...
public class ConfigOptionSearchAndRunHandler
  ...
  public ConfigOptionSearchAndRunHandler(
    ...
    // use the same output as the regular options handler to ensure that outputs
    // are all going in the same place. No need to do this for the exception
    // handler as we reuse it directly.
    this.useOut(resultHandler.out());
  }

  @Override
  public List<Object> handle(final ParseResult parseResult)
    ...
    commandLine.setDefaultValueProvider(
        createDefaultValueProvider(commandLine, configFile));
    commandLine.parseWithHandlers(
        resultHandler, exceptionHandler,
        parseResult.originalArgs().toArray(new String[0]));
    return new ArrayList<>();
  }

  private Optional<File> findConfigFile(final ParseResult parseResult,
                                        final CommandLine commandLine) {
    if (parseResult.hasMatchedOption(configFileOptionName)) {
      final OptionSpec configFileOption =
          parseResult.matchedOption(configFileOptionName);
      try {
  ...
  createDefaultValueProvider(final CommandLine commandLine,
