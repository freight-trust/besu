...
public class NewPendingTransactionFilterChangesCondition implements Condition {
  ...
  public NewPendingTransactionFilterChangesCondition(
      final EthFilterChangesTransaction filterChanges,
  ...
  public void verify(final Node node) {
    WaitUtils.waitFor(() -> {
      ...
      assertThat(response.getResult().size())
          .isEqualTo(transactionHashes.size());
      for (int i = 0; i < transactionHashes.size(); ++i) {
        assertThat(response.getLogs().get(0).get())
            .isEqualTo(transactionHashes.get(0));
      }
    });
  }
}
