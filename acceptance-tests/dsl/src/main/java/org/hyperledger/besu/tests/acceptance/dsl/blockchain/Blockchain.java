...
public class Blockchain {
  ...
  public Blockchain(final EthTransactions eth) { this.eth = eth; }
  ...
  public Condition reachesHeight(final BesuNode node,
                                 final int blocksAheadOfLatest) {
    return new ExpectBlockNumberAbove(eth,
                                      futureHeight(node, blocksAheadOfLatest));
  }

  public Condition reachesHeight(final BesuNode node,
                                 final int blocksAheadOfLatest,
                                 final int timeout) {
    return new ExpectBlockNumberAbove(
        eth, futureHeight(node, blocksAheadOfLatest), timeout);
  }
  ...
  private BigInteger futureHeight(final Node node,
                                  final int blocksAheadOfLatest) {
