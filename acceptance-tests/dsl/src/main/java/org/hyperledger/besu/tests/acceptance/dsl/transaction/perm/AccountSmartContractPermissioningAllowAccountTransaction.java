...
public class AccountSmartContractPermissioningAllowAccountTransaction
    implements Transaction<Hash> {

  private static final Bytes ADD_ACCOUNT_SIGNATURE =
      org.hyperledger.besu.crypto.Hash
          .keccak256(Bytes.of("addAccount(address)".getBytes(UTF_8)))
          .slice(0, 4);
  ...
  public AccountSmartContractPermissioningAllowAccountTransaction(
      final Account sender, final Address contractAddress,
  ...
  private String signedTransactionData() {
    final Bytes payload = Bytes.concatenate(
        ADD_ACCOUNT_SIGNATURE,
        Bytes.fromHexString("0x000000000000000000000000"), account);
