...
public class AddToOnChainPrivacyGroupTransaction
    implements Transaction<String> {
  private final Base64String privacyGroupId;
  private final PrivacyNode adder;
  private final List<String> addresses;

  public AddToOnChainPrivacyGroupTransaction(final String privacyGroupId,
                                             final PrivacyNode adder,
                                             final PrivacyNode... nodes) {
    this.privacyGroupId = Base64String.wrap(privacyGroupId);
    this.adder = adder;
    this.addresses = Arrays.stream(nodes)
  ...
  public String execute(final NodeRequests node) {
    try {
      return node.privacy().privxAddToPrivacyGroup(privacyGroupId, adder,
                                                   addresses);
