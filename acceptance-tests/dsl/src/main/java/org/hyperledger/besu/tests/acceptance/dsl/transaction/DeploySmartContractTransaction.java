...
public class DeploySmartContractTransaction<T extends Contract>
  ...
  public T execute(final NodeRequests node) {
    try {
      if (args != null && args.length != 0) {
        ...
        parameterObjects.addAll(Arrays.asList(
            node.eth(), BENEFACTOR_ONE, DEFAULT_GAS_PRICE, DEFAULT_GAS_LIMIT));
            ...
            Arrays.stream(clazz.getMethods())
                .filter(i
                        -> i.getName().equals("deploy") &&
                               parameterTypesAreEqual(i.getParameterTypes(),
                                                      parameterObjects))
                .findAny()
                .orElseThrow();

        final Object invoked =
            method.invoke(METHOD_IS_STATIC, parameterObjects.toArray());

        return cast(invoked).send();
      } else {
        final Method method =
            clazz.getMethod("deploy", Web3j.class, Credentials.class,
                            BigInteger.class, BigInteger.class);

        final Object invoked =
            method.invoke(METHOD_IS_STATIC, node.eth(), BENEFACTOR_ONE,
                          DEFAULT_GAS_PRICE, DEFAULT_GAS_LIMIT);
  ...
  private boolean
  parameterTypesAreEqual(final Class<?>[] expectedTypes,
                         final ArrayList<Object> actualObjects) {
