...
public class PrivCallTransaction implements Transaction<EthCall> {
  ...
  public PrivCallTransaction(final String privacyGroupId,
                             final Contract contract, final String encoded) {
  ...
  public EthCall execute(final NodeRequests node) {
    try {
      final EthCall response = node.privacy()
                                   .privCall(privacyGroupId, contract, encoded,
