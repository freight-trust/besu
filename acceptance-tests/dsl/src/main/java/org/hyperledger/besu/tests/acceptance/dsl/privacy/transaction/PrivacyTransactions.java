...
public class PrivacyTransactions {
  ...
  public PrivGetTransactionReceiptTransaction
  getPrivateTransactionReceipt(final String transactionHash) {
    return new PrivGetTransactionReceiptTransaction(transactionHash);
  }

  public CreatePrivacyGroupTransaction
  createPrivacyGroup(final String name, final String description,
  ...
  createOnChainPrivacyGroup(final PrivacyNode creator,
                            final PrivacyNode... nodes) {
    return new CreateOnChainPrivacyGroupTransaction(creator, nodes);
  }

  public AddToOnChainPrivacyGroupTransaction
  addToPrivacyGroup(final String privacyGroupId, final PrivacyNode adder,
                    final PrivacyNode... nodes) {
    return new AddToOnChainPrivacyGroupTransaction(privacyGroupId, adder,
                                                   nodes);
  }

  public LockOnChainPrivacyGroupTransaction
  privxLockPrivacyGroupAndCheck(final String privacyGroupId,
                                final PrivacyNode locker) {
    return new LockOnChainPrivacyGroupTransaction(privacyGroupId, locker);
  }

  public FindPrivacyGroupTransaction
  findPrivacyGroup(final List<String> nodes) {
    return new FindPrivacyGroupTransaction(nodes);
  }

  public FindOnChainPrivacyGroupTransaction
  findOnChainPrivacyGroup(final List<String> nodes) {
    return new FindOnChainPrivacyGroupTransaction(nodes);
  }

  public PrivDistributeTransactionTransaction
  ...
  public PrivCallTransaction privCall(final String privacyGroupId,
                                      final Contract contract,
                                      final String encoded) {
    return new PrivCallTransaction(privacyGroupId, contract, encoded);
  }

  public PrivGetCodeTransaction privGetCode(final String privacyGroupId,
                                            final Address contractAddress,
                                            final String blockParameter) {
    return new PrivGetCodeTransaction(privacyGroupId, contractAddress,
                                      blockParameter);
  }
  ...
  removeFromPrivacyGroup(final String privacyGroupId, final PrivacyNode remover,
                         final PrivacyNode nodeToRemove) {
    return new RemoveFromOnChainPrivacyGroupTransaction(privacyGroupId, remover,
                                                        nodeToRemove);
  }

  public EeaSendRawTransactionTransaction
  sendRawTransaction(final String transaction) {
    return new EeaSendRawTransactionTransaction(transaction);
  }

  public PrivNewFilterTransaction
  newFilter(final String privacyGroupId,
            final LogFilterJsonParameter filterParameter) {
    return new PrivNewFilterTransaction(privacyGroupId, filterParameter);
  }

  public PrivUninstallFilterTransaction
  uninstallFilter(final String privacyGroupId, final String filterId) {
    return new PrivUninstallFilterTransaction(privacyGroupId, filterId);
  }

  public PrivGetFilterLogsTransaction getFilterLogs(final String privacyGroupId,
                                                    final String filterId) {
    return new PrivGetFilterLogsTransaction(privacyGroupId, filterId);
  }

  public PrivGetFilterChangesTransaction
  getFilterChanges(final String privacyGroupId, final String filterId) {
