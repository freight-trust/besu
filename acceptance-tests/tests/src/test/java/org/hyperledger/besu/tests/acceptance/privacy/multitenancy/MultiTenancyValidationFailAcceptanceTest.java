...
public class MultiTenancyValidationFailAcceptanceTest
  ...
  public void setUp() throws Exception {
    ...
    final String token = node.execute(
        permissioningTransactions.createSuccessfulLogin("failUser", "pegasys"));
    ...
    final Transaction<Hash> transaction =
        privacyTransactions.sendRawTransaction(
            getRLPOutput(validSignedPrivateTransaction)
                .encoded()
                .toHexString());
    node.verify(priv.multiTenancyValidationFail(
        transaction,
  ...
  @Test
  public void
  sendRawTransactionShouldFailWhenPrivacyGroupDoesNotContainEnclaveKey()
      throws JsonProcessingException {
    final PrivateTransaction validSignedPrivateTransaction =
  ...
  @Test
  public void
  distributeRawTransactionShouldFailWhenPrivateFromNotMatchEnclaveKey() {
    final Address senderAddress = Address.wrap(
        Bytes.fromHexString(accounts.getPrimaryBenefactor().getAddress()));

    final Transaction<String> transaction =
        privacyTransactions.distributeRawTransaction(
            getRLPOutput(getValidSignedPrivateTransaction(
                             senderAddress, OTHER_ENCLAVE_PUBLIC_KEY))
                .encoded()
                .toHexString());
    node.verify(priv.multiTenancyValidationFail(
  ...
  @Test
  public void
  distributeRawTransactionShouldFailWhenPrivacyGroupDoesNotContainEnclaveKey()
      throws JsonProcessingException {
    final Address senderAddress = Address.wrap(
        Bytes.fromHexString(accounts.getPrimaryBenefactor().getAddress()));
  ...
  public void deletePrivacyGroupShouldFailWhenEnclaveKeyNotInPrivacyGroup()
    ...
    node.verify(priv.multiTenancyValidationFail(transaction,
                                                DELETE_PRIVACY_GROUP_ERROR));
  }

  @Test
  public void findPrivacyGroupShouldFailWhenEnclaveKeyNotInPrivacyGroup() {
    final Transaction<PrivacyGroup[]> transaction =
        privacyTransactions.findPrivacyGroup(
            new String[] {OTHER_ENCLAVE_PUBLIC_KEY});
    node.verify(
        priv.multiTenancyValidationFail(transaction, FIND_PRIVACY_GROUP_ERROR));
  }
  ...
  public void determineEeaNonceShouldFailWhenPrivateFromNotMatchEnclaveKey() {
    ...
    final String senderAddressBase64 =
        Bytes.fromHexString(accountAddress).toBase64String();
    ...
    node.verify(priv.multiTenancyValidationFail(
        transaction, GET_PRIVATE_TRANSACTION_NONCE_ERROR));
  }
  ...
  public void determineBesuNonceShouldFailWhenEnclaveKeyNotInPrivacyGroup()
    ...
    final String accountAddress = Address.ZERO.toHexString();
    final Transaction<Integer> transaction =
        privacyTransactions.getTransactionCount(accountAddress,
                                                PRIVACY_GROUP_ID);
    node.verify(priv.multiTenancyValidationFail(
        transaction, GET_PRIVATE_TRANSACTION_NONCE_ERROR));
  }
  ...
  private void retrievePrivacyGroupEnclaveStub()
    ...
    final String retrieveGroupResponse =
        mapper.writeValueAsString(testPrivacyGroup(
            List.of(OTHER_ENCLAVE_PUBLIC_KEY), PrivacyGroup.Type.PANTHEON));
    stubFor(
  ...
  getRLPOutput(final PrivateTransaction validSignedPrivateTransaction) {
