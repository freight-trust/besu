...
public interface WritePipe<T> {
  ...
  /**
   * Adds a new item to the pipe. This method will block until capacity is
   * available in the pipe. The item will be discarded if the pipe is closed
  ...
  /**
   * Close this write pipe indicating that no further data will be published to
   * it. When reading from the other end of this pipe {@link ReadPipe#hasMore()}
   * will continue to return true until all the already queued data has been
   * drained.
   */
  void close();
