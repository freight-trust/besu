...
public interface BlockHeader {
   ...
   *
   * @return The Keccak 256-bit hash of the parent block’s header, in its
   *     entirety.
   */
  Hash getParentHash();
   ...
   *
   * @return The 160-bit address to which all fees collected from the successful
   ...
   *
   * @return The Keccak 256-bit hash of the root node of the state trie, after
   *     all transactions are executed and finalisations applied.
   */
  Hash getStateRoot();

  /**
   * The Keccak 256-bit hash of theroot node of the trie structure populated
   * with each transaction in the transactions list portion of the block.
   *
   * @return The Keccak 256-bit hash of theroot node of the trie structure
   *     populated with each transaction in the transactions list portion of the
   *     block.
   */
   ...
   * the block.
   *
   * @return The Keccak 256-bit hash of the root node of the trie structure
   ...
   *
   * @return The Bloom filter composed from indexable information (logger
   *     address and log topics) contained in each log entry from the receipt of
   ...
   * timestamp.
   *
   * @return A UInt256 value corresponding to the difficulty level of this
   *     block. This can be calculated from the previous block’s difficulty
   ...
   *
   * @return A scalar value equal to the number of ancestor blocks. The genesis
   ...
   *
   * @return A scalar value equal to the current limit of gas expenditure per
   *     block.
   */
  long getGasLimit();
   ...
   *
   * @return A scalar value equal to the total gas used in transactions in this
   *     block.
   */
  long getGasUsed();
   ...
   * block’s inception.
   *
   * @return A scalar value equal to the reasonable output of Unix’s time() at
   ...
   *
   * @return An arbitrary byte array containing data relevant to this block.
   *     This must be 32 bytes or fewer.
   */
  Bytes getExtraData();

  /**
   * A 256-bit hash which, combined with the nonce, proves that a sufficient
   * amount of computation has been carried out on this block.
   *
   * @return A 256-bit hash which, combined with the nonce, proves that a
   *     sufficient amount of computation has been carried out on this block.
   */
  Hash getMixHash();

  /**
   * A 64-bit value which, combined with the mixhash, proves that a sufficient
   * amount of computation has been carried out on this block.
   *
   * @return A 64-bit value which, combined with the mixhash, proves that a
   *     sufficient amount of computation has been carried out on this block.
   */
  long getNonce();
