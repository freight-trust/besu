 ...
 * an actor externally to the scope of Ethereum. While it is assumed that the
 * ultimate external actor will be human in nature, software tools will be used
 * in its construction and dissemination.
 *
 * <p>There are two types of transactions: those which result in message calls
 * and those which result in the creation of new accounts with associated code
 * (known informally as ‘contract creation’). Message call transactions will
 * have an address present in the {@link #getTo} method whereas contract
 * creation transactions will not.
 */
public interface Transaction {
   ...
   * executing this transaction. This is paid up-front, before any computation
   * is done and may not be increased later.
   *
  ...
  /**
   * The 160-bit address of the message call’s recipient. For a contract
   * creation transaction this address will not be present.
   *
  ...
  /**
   * Value corresponding to the 'V' component of the signature of the
   * transaction.
   *
  ...
  /**
   * The chainId, computed from the 'V' portion of the signature. Used for
   * replay protection. If replay protection is not enabled this value will not
   * be present.
   *
   ...
   *
   * <p>Only present if this is a contract creation transaction, which is only
   * true if {@link #getTo} is empty.
   *
   ...
   *
   * <p>Only present if this is a message call transaction, which is only true
   * if {@link #getTo} is present.
   *
   ...
   *
   * <p>If this transaction is a message-call to an account (the {@link #getTo}
   * field is present), this same value will be exposed by {@link #getData}. If
   * instead this is a contract-creation transaction (the {@link #getTo} field
   * is absent), the payload is also exposed by {@link #getInit}.
   *
