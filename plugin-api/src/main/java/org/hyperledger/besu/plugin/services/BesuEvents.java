...
/**
 * This service allows plugins to attach to various events during the normal
 * operation of Besu.
 *
 * <p>Currently supported events
 *
 * <ul>
 *   <li><b>BlockAdded</b> - Fired when a new block has been evaluated and
 * validated. <li><b>BlockReorg</b> - Fired when a block is removed from the
 * chain to change to a different chainhead. <li><b>BlockPropagated</b> - Fired
 * when a new block header has been received and validated and is about to be
 * sent out to other peers, but before the body of the block has been evaluated
 *       and validated.
 *   <li><b>TransactionAdded</b> - Fired when a new transaction has been added
 * to the node. <li><b>TransactionDropped</b> - Fired when a new transaction has
 * been dropped from the node. <li><b>Logs</b> - Fired when a new block
 * containing logs is received. <li><b>SynchronizerStatus </b> - Fired when the
 * status of the synchronizer changes.
 * </ul>
 */
public interface BesuEvents {
   ...
   *
   * @param blockPropagatedListener The listener that will accept a BlockHeader
   *     as the event.
   * @return an id to be used as an identifier when de-registering the event.
   */
  long
  addBlockPropagatedListener(BlockPropagatedListener blockPropagatedListener);

  /**
   * Remove the blockAdded listener from besu notifications.
   *
   * @param listenerIdentifier The id that was returned from
   ...
   *
   * @param blockAddedListener The listener that will accept the Block object as
   *     the event.
   * @return an id to be used as an identifier when de-registering the event.
   */
   ...
   *
   * @param blockReorgListener The listener that will accept the reorg Block
   *     object as the event.
   * @return an id to be used as an identifier when de-registering the event.
   */
   ...
   *
   * @param transactionAddedListener The listener that will accept the
   *     Transaction object as the event.
   * @return an id to be used as an identifier when de-registering the event.
   */
  long addTransactionAddedListener(
      TransactionAddedListener transactionAddedListener);
   ...
   *
   * @param transactionDroppedListener The listener that will accept the
   *     Transaction object as the event.
   * @return an id to be used as an identifier when de-registering the event.
   */
  long addTransactionDroppedListener(
   ...
   *
   * @param syncStatusListener The listener that will accept the SyncStatus
   *     object as the event.
  ...
  /**
   * Add a listener that consumes every log (both added and removed) that
   * matches the filter parameters when a new block is added to the blockchain.
   *
   ...
   * @return The id of the listener to be referred to used to remove the
   *     listener.
   */
  long addLogListener(List<Address> addresses, List<List<Bytes32>> topics,
                      LogListener logListener);

  /**
   * Remove the log listener with the associated id.
   *
   * @param listenerIdentifier The id of the listener that was returned when the
  ...
  interface BlockPropagatedListener {
    ...
    /**
     * Invoked when a new block header has been received and validated and is
     * about to be sent out to other peers, but before the body of the block has
     * been evaluated and validated.
     *
     * <p>The block may not have been imported to the local chain yet and may
  ...
  interface LogListener {
    ...
    /**
     * Invoked for each log (both added and removed) when a new block is added
     * to the blockchain.
     *
